import{s as p,c as o,g as h,h as u}from"./index-DduhYjoe.js";import{g,e as $}from"./clientes-DqF6sVkj.js";let a=[],d="";async function b(){document.getElementById("tbacts").innerHTML="";const e=document.getElementById("content");e.innerHTML='<div class="empty">Carregando...</div>';const{data:n,error:i}=await p("clientes",{order:{column:"nome",ascending:!0}});if(i){e.innerHTML=`<div class="empty">Erro: ${i.message}</div>`;return}a=(n||[]).filter(r=>r.status==="ativo"),m()}function m(){let e=a;if(d){const t=d.toLowerCase();e=e.filter(s=>s.nome.toLowerCase().includes(t)||(s.empresa||"").toLowerCase().includes(t))}const n=e.reduce((t,s)=>t+Number(s.valor||0),0),i=e.length?n/e.length:0,r=e.length?e.map(t=>`

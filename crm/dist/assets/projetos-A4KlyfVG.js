import{s as I,d as f,o as O,f as E,t as u,h as F}from"./index-fpQzhZwW.js";const L=[{k:"todo",l:"A fazer",cor:"#A0A0A0"},{k:"doing",l:"Fazendo",cor:"#4A9EFF"},{k:"review",l:"Revisão",cor:"#F5A623"},{k:"done",l:"Feito",cor:"#34D399"}],M=[{k:"baixa",l:"Baixa",cor:"#A0A0A0"},{k:"media",l:"Média",cor:"#4A9EFF"},{k:"alta",l:"Alta",cor:"#F5A623"},{k:"urgente",l:"Urgente",cor:"#FF5C5C"}],B=Object.fromEntries(M.map(t=>[t.k,t])),q=["#C5F82A","#4A9EFF","#A78BFA","#F5A623","#34D399","#FF5C5C","#EC4899","#06B6D4"],S=["#A0A0A0","#4A9EFF","#A78BFA","#F5A623","#34D399","#FF5C5C","#EC4899","#06B6D4","#C5F82A"],G=[{k:1,l:"Seg"},{k:2,l:"Ter"},{k:3,l:"Qua"},{k:4,l:"Qui"},{k:5,l:"Sex"},{k:6,l:"Sáb"},{k:0,l:"Dom"}];let b=[],k=[],j=[],A=[],h=[],g=null,w="kanban";async function $(){const t=document.getElementById("content");t.innerHTML='<div class="empty">Carregando...</div>',await H(),await lt(),await H(),g&&!b.find(n=>n.id===g)&&(g=null),!g&&b.length&&(g=b[0].id),V(),K()}async function H(){const[t,n,i,a,d]=await Promise.all([I("projetos",{order:{column:"criado_em",ascending:!1}}),I("clientes",{columns:"id,nome,empresa,status",order:{column:"nome",ascending:!0}}),I("tarefas",{order:{column:"ordem",ascending:!0}}),I("tarefa_subtasks",{order:{column:"ordem",ascending:!0}}),I("rotinas",{order:{column:"criado_em",ascending:!1}})]);b=t.data||[],k=n.data||[],j=i.data||[],A=a.data||[],h=d.data||[]}function V(){const t=(d,o)=>`<button class="pj-tab${w===d?" on":""}" data-tab="${d}">${o}</button>`,n=w==="rotinas"?'<button class="btn bp" id="btn-add-rotina">+ Nova rotina</button>':'<button class="btn bp" id="btn-add-proj">+ Novo projeto</button>';document.getElementById("tbacts").innerHTML=`
  ...
  `,document.querySelectorAll(".pj-tab").forEach(d=>d.addEventListener("click",()=>{w=d.dataset.tab,V(),K()}));const i=document.getElementById("btn-add-proj"),a=document.getElementById("btn-add-rotina");i&&i.addEventListener("click",()=>R()),a&&a.addEventListener("click",()=>D())}function K(){return w==="hoje"?Z():w==="rotinas"?tt():Q()}function Z(){const t=document.getElementById("content"),n=new Date().toISOString().slice(0,10),i=j.filter(o=>o.status!=="done"&&o.prazo&&o.prazo<=n);if(!i.length){t.innerHTML=`<div class="empty" style="padding:80px 20px">
    ...
    </div>`}).join("");t.innerHTML=`<div class="pj-hoje">${d}</div>`,t.addEventListener("click",async o=>{const e=o.target.closest(".pj-hoje-cb");if(e){o.stopPropagation();const l=j.find(r=>r.id===e.dataset.tid);if(!l)return;l.status="done",await f.from("tarefas").update({status:"done",atualizado_em:new Date().toISOString()}).eq("id",l.id),u("Marcada como feita"),$();return}const s=o.target.closest(".pj-hoje-row");if(s){const l=j.find(r=>r.id===s.dataset.tid);l&&z(l)}})}function tt(){const t=document.getElementById("content");if(!h.length){t.innerHTML=`<div class="empty" style="padding:80px 20px">
  ...
  </div>`}function it(t){const n=A.filter(d=>d.tarefa_id===t.id),i=n.filter(d=>d.feito).length,a=B[t.prioridade]||B.media;return`<div class="pj-card" draggable="true" data-tid="${t.id}">
  ...
  </div>`}async function X(t){const{error:n}=await f.from("tarefas").delete().eq("id",t);if(n)return u("Erro: "+n.message,"err");u("Tarefa excluída"),$()}function D(t={}){const n=!t.id,i='<option value="">— sem cliente —</option>'+Y(t.cliente_id).map(r=>`<option value="${r.id}"${t.cliente_id===r.id?" selected":""}>${m(r.empresa||r.nome)}</option>`).join(""),a='<option value="">— escolha um projeto —</option>'+b.map(r=>`<option value="${r.id}"${t.projeto_id===r.id?" selected":""}>${m(r.nome)}</option>`).join(""),d=t.cadencia||"semanal",o=t.dias_semana||[1],e=Array.isArray(t.tarefas)?JSON.parse(JSON.stringify(t.tarefas)):[],s=G.map(r=>`<label class="pj-dia-chip ${o.includes(r.k)?"on":""}" data-d="${r.k}"><input type="checkbox" ${o.includes(r.k)?"checked":""} style="display:none">${r.l}</label>`).join("");O(n?"Nova rotina":"Editar rotina",`
    <div class="fg" style="margin-bottom:11px"><label class="fl">Nome *</label>

import{s as A,d as g,o as z,f as x,t as f,h as R,k as at}from"./index-7SRJSBHj.js";const it="https://flzpblpegoqjxaacjvhf.supabase.co";async function U(t,i){var e;const{data:n}=await at.auth.getSession(),o=(e=n==null?void 0:n.session)==null?void 0:e.access_token,s=await fetch(`${it}/functions/v1/${t}`,{method:"POST",headers:{"Content-Type":"application/json",...o?{Authorization:`Bearer ${o}`}:{}},body:JSON.stringify(i||{})}),r=await s.text();let a;try{a=JSON.parse(r)}catch{a={ok:!1,erro:r}}if(!s.ok||(a==null?void 0:a.ok)===!1)throw new Error((a==null?void 0:a.erro)||`HTTP ${s.status}`);return a}const q=[{k:"todo",l:"A fazer",cor:"#A0A0A0"},{k:"doing",l:"Fazendo",cor:"#4A9EFF"},{k:"review",l:"Revisão",cor:"#F5A623"},{k:"done",l:"Feito",cor:"#34D399"}],N=[{k:"baixa",l:"Baixa",cor:"#A0A0A0"},{k:"media",l:"Média",cor:"#4A9EFF"},{k:"alta",l:"Alta",cor:"#F5A623"},{k:"urgente",l:"Urgente",cor:"#FF5C5C"}],T=Object.fromEntries(N.map(t=>[t.k,t])),P=["#C5F82A","#4A9EFF","#A78BFA","#F5A623","#34D399","#FF5C5C","#EC4899","#06B6D4"],L=["#A0A0A0","#4A9EFF","#A78BFA","#F5A623","#34D399","#FF5C5C","#EC4899","#06B6D4","#C5F82A"],K=[{k:1,l:"Seg"},{k:2,l:"Ter"},{k:3,l:"Qua"},{k:4,l:"Qui"},{k:5,l:"Sex"},{k:6,l:"Sáb"},{k:0,l:"Dom"}];let j=[],S=[],$=[],B=[],_=[],h=null,I="kanban";async function w(){const t=document.getElementById("content");t.innerHTML='<div class="empty">Carregando...</div>',await W(),await ft(),await W(),h&&!j.find(i=>i.id===h)&&(h=null),!h&&j.length&&(h=j[0].id),Q(),V()}async function W(){const[t,i,n,o,s]=await Promise.all([A("projetos",{order:{column:"criado_em",ascending:!1}}),A("clientes",{columns:"id,nome,empresa,status,whatsapp",order:{column:"nome",ascending:!0}}),A("tarefas",{order:{column:"ordem",ascending:!0}}),A("tarefa_subtasks",{order:{column:"ordem",ascending:!0}}),A("rotinas",{order:{column:"criado_em",ascending:!1}})]);j=t.data||[],S=i.data||[],$=n.data||[],B=o.data||[],_=s.data||[]}function Q(){const t=(s,r)=>`<button class="pj-tab${I===s?" on":""}" data-tab="${s}">${r}</button>`,i=I==="rotinas"?'<button class="btn bp" id="btn-add-rotina">+ Nova rotina</button>':'<button class="btn bp" id="btn-add-proj">+ Novo projeto</button>';document.getElementById("tbacts").innerHTML=`
    <div class="pj-tabs">${t("kanban","Kanban")}${t("lista","Lista")}${t("hoje","Hoje")}${t("resumao","Resumão")}${t("rotinas","Rotinas")}</div>
    ${i}
  `,document.querySelectorAll(".pj-tab").forEach(s=>s.addEventListener("click",()=>{I=s.dataset.tab,Q(),V()}));const n=document.getElementById("btn-add-proj"),o=document.getElementById("btn-add-rotina");n&&n.addEventListener("click",()=>J()),o&&o.addEventListener("click",()=>C())}function V(){return I==="hoje"?ot():I==="rotinas"?nt():I==="resumao"?M():X()}async function M(){const t=document.getElementById("content");t.innerHTML='<div class="empty">Carregando resumão…</div>';let i;try{i=await U("digest-cliente",{})}catch(s){t.innerHTML=`<div class="empty">Erro ao carregar: ${v(s.message)}</div>`;return}const n=(i==null?void 0:i.clientes)||[];if(!n.length){t.innerHTML=`
...
Qualquer dúvida é só chamar.`;z(`Enviar pra ${a.nome}?`,`<div style="font-size:12px;color:var(--text-3);margin-bottom:8px">Texto final que vai pelo WhatsApp:</div>
    ...
    </div>`;return}const o={};for(const r of n)(o[r.projeto_id]=o[r.projeto_id]||[]).push(r);const s=Object.entries(o).map(([r,a])=>{const e=j.find(l=>l.id===r);if(!e)return"";const c=S.find(l=>l.id===e.cliente_id),d=a.sort((l,p)=>(l.prazo||"").localeCompare(p.prazo||"")).map(l=>`<div class="pj-hoje-row" data-tid="${l.id}">
    ...
    </div>`}).join("");t.innerHTML=`<div class="pj-hoje">${s}</div>`,t.addEventListener("click",async r=>{const a=r.target.closest(".pj-hoje-cb");if(a){r.stopPropagation();const c=$.find(d=>d.id===a.dataset.tid);if(!c)return;await Z(c,()=>w());return}const e=r.target.closest(".pj-hoje-row");if(e){const c=$.find(d=>d.id===e.dataset.tid);c&&O(c)}})}function nt(){const t=document.getElementById("content");if(!_.length){t.innerHTML=`<div class="empty" style="padding:80px 20px">
      ...
      <div class="pj-rot-item-main">
        ...
        <div class="pj-rot-item-meta">
          ...
          ${e.ultima_geracao?`<span class="pj-rot-chip pj-rot-chip-dim">Última: ${R(e.ultima_geracao)}</span>`:""}
        </div>
  ...
  </div>`:"";t.innerHTML=`<div class="pj-rot-groups">${r}${a}</div>`,t.addEventListener("click",async e=>{const c=e.target.closest(".rot-add-proj");if(c){C({projeto_id:c.dataset.pid});return}const d=e.target.closest(".rot-edit"),l=e.target.closest(".rot-del"),p=e.target.closest(".rot-run"),b=e.target.closest(".pj-rot-active");if(d){const u=_.find(m=>m.id===d.dataset.rid);u&&C(u)}else if(l){const u=_.find(m=>m.id===l.dataset.rid);u&&confirm(`Excluir rotina "${u.nome}"? (Tarefas já geradas continuam)`)&&(await g.from("rotinas").delete().eq("id",u.id),f("Rotina excluída"),w())}else if(p){const u=_.find(m=>m.id===p.dataset.rid);u&&(await vt(u),f("Tarefas geradas"),w())}else if(b){const u=_.find(m=>m.id===b.dataset.rid);u&&(u.ativa=b.checked,await g.from("rotinas").update({ativa:u.ativa,atualizado_em:new Date().toISOString()}).eq("id",u.id))}})}function st(t){if(t.cadencia==="diaria")return"Todos os dias";if(t.cadencia==="semanal"){const i=(t.dias_semana||[]).map(n=>{var o;return(o=K.find(s=>s.k===n))==null?void 0:o.l}).filter(Boolean).join(", ");return i?`Semanal: ${i}`:"Semanal"}return t.cadencia==="mensal"?`Mensal: dia ${t.dia_mes||1}`:t.cadencia}function X(){const t=document.getElementById("content");if(!j.length){t.innerHTML=`<div class="empty" style="padding:80px 20px">
    ...
    </div>`,document.getElementById("btn-add-first").addEventListener("click",()=>J());return}const i=j.map(n=>{const o=S.find(a=>a.id===n.cliente_id),s=$.filter(a=>a.projeto_id===n.id).length,r=$.filter(a=>a.projeto_id===n.id&&a.status==="done").length;return`<div class="pj-side ${n.id===h?"on":""}" data-pid="${n.id}">
  ...
  </div>`,t.querySelector(".pj-sidebar-list").addEventListener("click",n=>{const o=n.target.closest(".pj-side");o&&(h=o.dataset.pid,X())}),F()}function F(){const t=j.find(a=>a.id===h);if(!t)return;const i=S.find(a=>a.id===t.cliente_id),n=$.filter(a=>a.projeto_id===t.id),o=D(t),s=`<div class="pj-head">
    <div class="pj-head-left">
      ...
      <div>
        ...
        <div class="pj-head-meta">
          ...
          ${t.prazo?` · <span>Prazo ${R(t.prazo)}</span>`:""}
        </div>
      </div>
    </div>
  ...
  </div>`,r=I==="lista"?dt(n,o):rt(n,o);document.getElementById("pj-main").innerHTML=s+r,document.getElementById("pj-edit").addEventListener("click",()=>J(t)),document.getElementById("pj-del").addEventListener("click",()=>ut(t)),document.getElementById("pj-add-tar").addEventListener("click",()=>{var a;return O({projeto_id:t.id,status:((a=o[0])==null?void 0:a.k)||"todo"})}),pt()}function D(t){const i=t==null?void 0:t.etapas;return Array.isArray(i)&&i.length?i:q}function rt(t,i){return`<div class="pj-kanban">${i.map(o=>{const s=t.filter(a=>a.status===o.k),r=s.length?s.map(a=>lt(a)).join(""):'<div class="pj-empty">—</div>';return`<div class="pj-col" data-status="${o.k}">
    ...
    </div>`}).join("")}</div>`}function dt(t,i){if(!t.length)return'<div class="empty" style="padding:40px">Sem tarefas neste projeto.</div>';const n=i.map(r=>r.k);return`<div class="tw" style="margin-top:14px">
    <table>
      <thead><tr><th>Status</th><th>Tarefa</th><th>Prio</th><th>Prazo</th><th></th></tr></thead>
      <tbody>${[...t].sort((r,a)=>n.indexOf(r.status)-n.indexOf(a.status)||(r.prazo||"").localeCompare(a.prazo||"")).map(r=>{const a=B.filter(c=>c.tarefa_id===r.id),e=a.filter(c=>c.feito).length;return`<tr class="pj-row" data-tid="${r.id}">
      <td>${ct(r.status,i)}</td>
      ...
      <td>${Y(r.prioridade)}</td>
      <td class="tm">${H(r.prazo)}</td>
  ...
  </div>`}function ct(t,i){const n=(i||q).find(o=>o.k===t)||q[0];return`<span class="pj-pill" style="background:${n.cor}22;color:${n.cor}">${v(n.l)}</span>`}function Y(t){const i=T[t]||T.media;return`<span class="pj-pill" style="background:${i.cor}22;color:${i.cor}">${i.l}</span>`}function H(t){if(!t)return"";const i=new Date(t+"T00:00:00"),n=new Date;n.setHours(0,0,0,0);const o=Math.round((i-n)/864e5);let s="pj-prazo";return o<0?s+=" late":o<=2&&(s+=" soon"),`<span class="${s}">${R(t)}</span>`}function pt(){const t=document.getElementById("pj-main");if(!t)return;t.addEventListener("click",n=>{const o=n.target.closest(".pj-card"),s=n.target.closest(".pj-row"),r=n.target.closest(".del-tar");if(r){n.stopPropagation();const a=$.find(e=>e.id===r.dataset.tid);a&&confirm(`Excluir "${a.titulo}"?`)&&tt(a.id);return}if(o){const a=$.find(e=>e.id===o.dataset.tid);a&&O(a)}else if(s){const a=$.find(e=>e.id===s.dataset.tid);a&&O(a)}});let i=null;t.querySelectorAll(".pj-card").forEach(n=>{n.addEventListener("dragstart",o=>{i=n.dataset.tid,n.classList.add("drag"),o.dataTransfer.effectAllowed="move"}),n.addEventListener("dragend",()=>{n.classList.remove("drag"),t.querySelectorAll(".pj-col-cards.over").forEach(o=>o.classList.remove("over"))})}),t.querySelectorAll(".pj-col-cards").forEach(n=>{n.addEventListener("dragover",o=>{o.preventDefault(),n.classList.add("over")}),n.addEventListener("dragleave",()=>n.classList.remove("over")),n.addEventListener("drop",async o=>{if(o.preventDefault(),n.classList.remove("over"),!i)return;const s=n.dataset.drop,r=$.find(e=>e.id===i);if(!r||r.status===s)return;if(s==="done"){await Z(r,()=>F());return}r.status=s;const{error:a}=await g.from("tarefas").update({status:s,atualizado_em:new Date().toISOString()}).eq("id",i);if(a){f("Erro: "+a.message,"err");return}F()})})}function J(t={}){const i='<option value="">— sem cliente (interno) —</option>'+et(t.cliente_id).map(a=>`<option value="${a.id}"${t.cliente_id===a.id?" selected":""}>${v(a.empresa||a.nome)}</option>`).join(""),n=P.map(a=>`<span class="pj-cor-opt${(t.cor||P[0])===a?" on":""}" data-cor="${a}" style="background:${a}"></span>`).join("");let o=Array.isArray(t.etapas)&&t.etapas.length?JSON.parse(JSON.stringify(t.etapas)):JSON.parse(JSON.stringify(q));z(t.id?"Editar projeto":"Novo projeto",`
  ...
  `);let s=t.cor||P[0];document.getElementById("pj-cor-row").addEventListener("click",a=>{const e=a.target.closest(".pj-cor-opt");e&&(s=e.dataset.cor,document.querySelectorAll(".pj-cor-opt").forEach(c=>c.classList.toggle("on",c.dataset.cor===s)))});const r=()=>{document.getElementById("pj-etapas-list").innerHTML=o.map((a,e)=>`
    ...
    <div class="fg" style="margin-bottom:11px">
      ...
      <input class="fi" id="cn-apelido" value="${E(r)}" placeholder="Ex: Criativo de feed dessa semana">
  ...
  </div>`}async function tt(t){const{error:i}=await g.from("tarefas").delete().eq("id",t);if(i)return f("Erro: "+i.message,"err");f("Tarefa excluída"),w()}function C(t={}){const i=!t.id,n='<option value="">— sem cliente —</option>'+et(t.cliente_id).map(d=>`<option value="${d.id}"${t.cliente_id===d.id?" selected":""}>${v(d.empresa||d.nome)}</option>`).join(""),o='<option value="">— escolha um projeto —</option>'+j.map(d=>`<option value="${d.id}"${t.projeto_id===d.id?" selected":""}>${v(d.nome)}</option>`).join(""),s=t.cadencia||"semanal",r=t.dias_semana||[1],a=Array.isArray(t.tarefas)?JSON.parse(JSON.stringify(t.tarefas)):[],e=K.map(d=>`<label class="pj-dia-chip ${r.includes(d.k)?"on":""}" data-d="${d.k}"><input type="checkbox" ${r.includes(d.k)?"checked":""} style="display:none">${d.l}</label>`).join("");z(i?"Nova rotina":"Editar rotina",`
    <div class="fg" style="margin-bottom:11px"><label class="fl">Nome *</label>
  ...
  `);const c=()=>{document.getElementById("rt-tars").innerHTML=a.map((d,l)=>{const p=d.notificar_cliente!==!1;return`
      <div class="pj-rot-tar-row" data-i="${l}" style="flex-direction:column;align-items:stretch;gap:6px;padding:10px;border:1px solid var(--line);border-radius:var(--rs);margin-bottom:6px">
        <div style="display:flex;gap:6px;align-items:center">
          <input type="text" class="fi rt-t-titulo" data-i="${l}" value="${E(d.titulo||"")}" placeholder="Título interno" style="flex:2;padding:6px 9px;font-size:12.5px">
          <select class="fsl rt-t-prio" data-i="${l}" style="flex:1;padding:6px 9px;font-size:12.5px">
            ${N.map(b=>`<option value="${b.k}"${(d.prioridade||"media")===b.k?" selected":""}>${b.l}</option>`).join("")}
          </select>
          <button class="pj-sub-del" type="button" data-i="${l}">×</button>
        </div>
        <input type="text" class="fi rt-t-apelido" data-i="${l}" value="${E(d.apelido_cliente||"")}" placeholder="Como mostrar pro cliente (opcional)" style="padding:6px 9px;font-size:12.5px">
      ...
      </div>`}).join("")};c(),document.getElementById("rt-tar-add").addEventListener("click",()=>{a.push({titulo:"",prioridade:"media",apelido_cliente:"",notificar_cliente:!0}),c()}),document.getElementById("rt-tars").addEventListener("click",d=>{const l=d.target.closest(".pj-sub-del");l&&(a.splice(+l.dataset.i,1),c())}),document.getElementById("rt-tars").addEventListener("input",d=>{const l=d.target.closest(".rt-t-titulo"),p=d.target.closest(".rt-t-prio"),b=d.target.closest(".rt-t-apelido");l&&(a[+l.dataset.i].titulo=l.value),p&&(a[+p.dataset.i].prioridade=p.value),b&&(a[+b.dataset.i].apelido_cliente=b.value)}),document.getElementById("rt-tars").addEventListener("change",d=>{const l=d.target.closest(".rt-t-notif");l&&(a[+l.dataset.i].notificar_cliente=l.checked)}),document.querySelectorAll('input[name="rt-cad"]').forEach(d=>d.addEventListener("change",()=>{document.querySelectorAll(".pj-cad-opt").forEach(p=>p.classList.toggle("on",p.querySelector("input").checked));const l=document.querySelector('input[name="rt-cad"]:checked').value;document.getElementById("rt-dias-wrap").style.display=l==="semanal"?"":"none",document.getElementById("rt-mes-wrap").style.display=l==="mensal"?"":"none"})),document.getElementById("rt-dias").addEventListener("click",d=>{const l=d.target.closest(".pj-dia-chip");l&&(d.preventDefault(),l.classList.toggle("on"),l.querySelector("input").checked=l.classList.contains("on"))}),document.getElementById("rt-cancel").addEventListener("click",x),i||document.getElementById("rt-del").addEventListener("click",async()=>{confirm(`Excluir rotina "${t.nome}"?`)&&(await g.from("rotinas").delete().eq("id",t.id),x(),f("Rotina excluída"),w())}),document.getElementById("rt-save").addEventListener("click",async()=>{const d=document.querySelector('input[name="rt-cad"]:checked').value,l=[...document.querySelectorAll("#rt-dias .pj-dia-chip.on")].map(m=>+m.dataset.d),p={nome:document.getElementById("rt-nome").value.trim(),cliente_id:document.getElementById("rt-cli").value||null,projeto_id:document.getElementById("rt-proj").value||null,cadencia:d,dias_semana:d==="semanal"?l:[],dia_mes:d==="mensal"?+document.getElementById("rt-dia-mes").value||1:null,tarefas:a.filter(m=>(m.titulo||"").trim()),atualizado_em:new Date().toISOString()};if(!p.nome)return f("Nome é obrigatório","err");if(!p.projeto_id)return f("Selecione um projeto","err");if(d==="semanal"&&!p.dias_semana.length)return f("Marque pelo menos 1 dia","err");if(!p.tarefas.length)return f("Adicione pelo menos 1 tarefa","err");const b=t.id?g.from("rotinas").update(p).eq("id",t.id):g.from("rotinas").insert({...p,ativa:!0}),{error:u}=await b;if(u)return f("Erro: "+u.message,"err");x(),f(t.id?"Rotina salva":"Rotina criada"),w()})}function mt(t,i){const n=i.toISOString().slice(0,10);return t.ultima_geracao===n?!1:t.cadencia==="diaria"?!0:t.cadencia==="semanal"?(t.dias_semana||[]).includes(i.getDay()):t.cadencia==="mensal"?i.getDate()===(t.dia_mes||1):!1}async function ft(){var n;const t=new Date,i=t.toISOString().slice(0,10);for(const o of _){if(!o.ativa||!o.projeto_id||!mt(o,t))continue;const s=j.find(e=>e.id===o.projeto_id),r=((n=D(s)[0])==null?void 0:n.k)||"todo",a=(o.tarefas||[]).filter(e=>(e.titulo||"").trim()).map(e=>({projeto_id:o.projeto_id,rotina_id:o.id,titulo:e.titulo,descricao:e.descricao||`[Gerada pela rotina "${o.nome}"]`,prioridade:e.prioridade||"media",status:r,prazo:i,apelido_cliente:(e.apelido_cliente||"").trim()||null,notificar_cliente:e.notificar_cliente!==!1}));a.length&&(await g.from("tarefas").insert(a),await g.from("rotinas").update({ultima_geracao:i}).eq("id",o.id))}}async function vt(t){var a;if(!t.projeto_id){f("Rotina sem projeto","err");return}const i=j.find(e=>e.id===t.projeto_id),n=((a=D(i)[0])==null?void 0:a.k)||"todo",o=new Date().toISOString().slice(0,10),{count:s}=await g.from("tarefas").select("id",{count:"exact",head:!0}).eq("rotina_id",t.id).eq("prazo",o);if((s||0)>0&&!confirm(`Essa rotina já rodou hoje (${s} tarefa${s===1?"":"s"} já gerada${s===1?"":"s"}). Rodar de novo vai duplicar. Continuar mesmo assim?`)){f("Cancelado.","er");return}const r=(t.tarefas||[]).filter(e=>(e.titulo||"").trim()).map(e=>({projeto_id:t.projeto_id,rotina_id:t.id,titulo:e.titulo,descricao:e.descricao||`[Gerada pela rotina "${t.nome}" — manual]`,prioridade:e.prioridade||"media",status:n,prazo:o,apelido_cliente:(e.apelido_cliente||"").trim()||null,notificar_cliente:e.notificar_cliente!==!1}));if(!r.length){f("Rotina sem tarefas","err");return}await g.from("tarefas").insert(r),await g.from("rotinas").update({ultima_geracao:o}).eq("id",t.id)}function et(t){const i=new Set(["proposta","ativo","em_pausa","fechado"]);return S.filter(n=>i.has(n.status)||n.id===t)}function v(t){return String(t??"").replace(/[&<>"']/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[i])}function E(t){return v(t)}export{w as render};

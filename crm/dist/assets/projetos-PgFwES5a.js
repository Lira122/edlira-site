import{s as A,d as g,o as z,f as x,t as f,h as N,k as at}from"./index-Bfk_R2KK.js";const it="https://flzpblpegoqjxaacjvhf.supabase.co";async function M(t,i){var e;const{data:s}=await at.auth.getSession(),o=(e=s==null?void 0:s.session)==null?void 0:e.access_token,n=await fetch(`${it}/functions/v1/${t}`,{method:"POST",headers:{"Content-Type":"application/json",...o?{Authorization:`Bearer ${o}`}:{}},body:JSON.stringify(i||{})}),d=await n.text();let a;try{a=JSON.parse(d)}catch{a={ok:!1,erro:d}}if(!n.ok||(a==null?void 0:a.ok)===!1)throw new Error((a==null?void 0:a.erro)||`HTTP ${n.status}`);return a}const q=[{k:"todo",l:"A fazer",cor:"#A0A0A0"},{k:"doing",l:"Fazendo",cor:"#4A9EFF"},{k:"review",l:"Revisão",cor:"#F5A623"},{k:"done",l:"Feito",cor:"#34D399"}],H=[{k:"baixa",l:"Baixa",cor:"#A0A0A0"},{k:"media",l:"Média",cor:"#4A9EFF"},{k:"alta",l:"Alta",cor:"#F5A623"},{k:"urgente",l:"Urgente",cor:"#FF5C5C"}],T=Object.fromEntries(H.map(t=>[t.k,t])),P=["#C5F82A","#4A9EFF","#A78BFA","#F5A623","#34D399","#FF5C5C","#EC4899","#06B6D4"],L=["#A0A0A0","#4A9EFF","#A78BFA","#F5A623","#34D399","#FF5C5C","#EC4899","#06B6D4","#C5F82A"],U=[{k:1,l:"Seg"},{k:2,l:"Ter"},{k:3,l:"Qua"},{k:4,l:"Qui"},{k:5,l:"Sex"},{k:6,l:"Sáb"},{k:0,l:"Dom"}];let j=[],S=[],$=[],B=[],w=[],h=null,I="kanban";async function _(){const t=document.getElementById("content");t.innerHTML='<div class="empty">Carregando...</div>',await K(),await ft(),await K(),h&&!j.find(i=>i.id===h)&&(h=null),!h&&j.length&&(h=j[0].id),V(),W()}async function K(){const[t,i,s,o,n]=await Promise.all([A("projetos",{order:{column:"criado_em",ascending:!1}}),A("clientes",{columns:"id,nome,empresa,status,whatsapp",order:{column:"nome",ascending:!0}}),A("tarefas",{order:{column:"ordem",ascending:!0}}),A("tarefa_subtasks",{order:{column:"ordem",ascending:!0}}),A("rotinas",{order:{column:"criado_em",ascending:!1}})]);j=t.data||[],S=i.data||[],$=s.data||[],B=o.data||[],w=n.data||[]}function V(){const t=(n,d)=>`<button class="pj-tab${I===n?" on":""}" data-tab="${n}">${d}</button>`,i=I==="rotinas"?'<button class="btn bp" id="btn-add-rotina">+ Nova rotina</button>':'<button class="btn bp" id="btn-add-proj">+ Novo projeto</button>';document.getElementById("tbacts").innerHTML=`
    <div class="pj-tabs">${t("kanban","Kanban")}${t("lista","Lista")}${t("hoje","Hoje")}${t("resumao","Resumão")}${t("rotinas","Rotinas")}</div>
    ${i}
  `,document.querySelectorAll(".pj-tab").forEach(n=>n.addEventListener("click",()=>{I=n.dataset.tab,V(),W()}));const s=document.getElementById("btn-add-proj"),o=document.getElementById("btn-add-rotina");s&&s.addEventListener("click",()=>G()),o&&o.addEventListener("click",()=>C())}function W(){return I==="hoje"?ot():I==="rotinas"?nt():I==="resumao"?F():X()}async function F(){const t=document.getElementById("content");t.innerHTML='<div class="empty">Carregando resumão…</div>';let i;try{i=await M("digest-cliente",{})}catch(n){t.innerHTML=`<div class="empty">Erro ao carregar: ${v(n.message)}</div>`;return}const s=(i==null?void 0:i.clientes)||[];if(!s.length){t.innerHTML=`
      ...
      </div>`;return}const o=n=>{const d=n.tarefas.map(a=>`
      <tr data-tid="${a.id}">
        <td style="width:30px">
      ...
      <div class="tw" data-cid="${n.cliente_id}" style="margin-bottom:18px">
        <div class="th" style="align-items:flex-start">
          <div>
            <h3 style="font-size:14px">${v(n.nome)}</h3>
            <div style="font-size:11px;color:var(--text-3);margin-top:3px">${n.tarefas.length} tarefa${n.tarefas.length===1?"":"s"} pendente${n.tarefas.length===1?"":"s"} · ${v(n.whatsapp||"")}</div>
          </div>
          <div style="display:flex;gap:8px">
            <button class="btn bg bsm rs-skip" data-cid="${n.cliente_id}">Pular hoje</button>
            <button class="btn bp bsm rs-send" data-cid="${n.cliente_id}">Revisar e enviar →</button>
          </div>
        </div>
        <table>
          <thead>
    ...
    <div style="font-size:13px;color:var(--text-3);margin-bottom:16px;line-height:1.5">
      ${s.length} cliente${s.length===1?"":"s"} esperando aviso.
    ...
    ${s.map(o).join("")}
  `,t.querySelectorAll(".rs-send").forEach(n=>{n.addEventListener("click",async()=>{const d=n.dataset.cid,a=s.find(m=>m.cliente_id===d);if(!a)return;const e=t.querySelector(`.tw[data-cid="${d}"]`),c=[...e.querySelectorAll(".rs-tar-cb")].filter(m=>m.checked).map(m=>m.dataset.tid);if(!c.length){f("Nenhuma tarefa marcada.","err");return}const r={};c.forEach(m=>{const y=e.querySelector(`.rs-apelido[data-tid="${m}"]`);y&&(r[m]=y.value.trim())});const l=(a.nome||"").split(" ")[0]||"",p=l?`Oi, ${l}! Fechando o dia aqui.`:"Oi! Fechando o dia aqui.",b=c.map(m=>{var y;return`✓ ${r[m]||((y=a.tarefas.find(k=>k.id===m))==null?void 0:y.titulo)||""}`}).join(`
`),u=`${p}
...
Qualquer dúvida é só chamar.`;z(`Enviar pra ${a.nome}?`,`<div style="font-size:12px;color:var(--text-3);margin-bottom:8px">Texto final que vai pelo WhatsApp:</div>
         ...
         <button class="btn bp" id="rs-confirm">Confirmar envio</button>`),document.getElementById("rs-cancel").addEventListener("click",x),document.getElementById("rs-confirm").addEventListener("click",async()=>{const m=document.getElementById("rs-confirm");m.disabled=!0,m.textContent="Enviando…";try{const y=a.tarefas.filter(k=>!c.includes(k.id)).map(k=>k.id);for(const k of y)await g.from("tarefas").update({notificar_cliente:!1}).eq("id",k);await M("digest-cliente",{cliente_id:d,apelidos:r}),f(`Enviado pra ${a.nome} ✓`),x(),F()}catch(y){m.disabled=!1,m.textContent="Confirmar envio",f("Erro: "+y.message,"err")}})})}),t.querySelectorAll(".rs-skip").forEach(n=>{n.addEventListener("click",async()=>{const d=n.dataset.cid,a=s.find(r=>r.cliente_id===d);if(!a||!confirm(`Pular ${a.nome} hoje? As tarefas somem do resumão e não viram aviso.`))return;const e=a.tarefas.map(r=>r.id),c=new Date().toISOString();await g.from("tarefas").update({notificado_cliente_em:c}).in("id",e),f(`${a.nome} pulado`),F()})})}function ot(){const t=document.getElementById("content"),i=new Date().toISOString().slice(0,10),s=$.filter(d=>d.status!=="done"&&d.prazo&&d.prazo<=i);if(!s.length){t.innerHTML=`<div class="empty" style="padding:80px 20px">
    ...
    </div>`;return}const o={};for(const d of s)(o[d.projeto_id]=o[d.projeto_id]||[]).push(d);const n=Object.entries(o).map(([d,a])=>{const e=j.find(l=>l.id===d);if(!e)return"";const c=S.find(l=>l.id===e.cliente_id),r=a.sort((l,p)=>(l.prazo||"").localeCompare(p.prazo||"")).map(l=>`<div class="pj-hoje-row" data-tid="${l.id}">
    ...
    </div>`}).join("");t.innerHTML=`<div class="pj-hoje">${n}</div>`,t.addEventListener("click",async d=>{const a=d.target.closest(".pj-hoje-cb");if(a){d.stopPropagation();const c=$.find(r=>r.id===a.dataset.tid);if(!c)return;await Z(c,()=>_());return}const e=d.target.closest(".pj-hoje-row");if(e){const c=$.find(r=>r.id===e.dataset.tid);c&&O(c)}})}function nt(){const t=document.getElementById("content");if(!w.length){t.innerHTML=`<div class="empty" style="padding:80px 20px">
    ...
    </div>`,document.getElementById("btn-first-rot").addEventListener("click",()=>C());return}const i=new Map,s=[];w.forEach(e=>{if(!e.projeto_id){s.push(e);return}i.has(e.projeto_id)||i.set(e.projeto_id,[]),i.get(e.projeto_id).push(e)});const o=[...j].filter(e=>i.has(e.id)).sort((e,c)=>(e.nome||"").localeCompare(c.nome||"")),n=e=>{const c=S.find(l=>l.id===e.cliente_id),r=Array.isArray(e.tarefas)?e.tarefas:[];return`<div class="pj-rot-item ${e.ativa?"":"paused"}" data-rid="${e.id}">
    ...
    </div>`,document.getElementById("btn-add-first").addEventListener("click",()=>G());return}const i=j.map(s=>{const o=S.find(a=>a.id===s.cliente_id),n=$.filter(a=>a.projeto_id===s.id).length,d=$.filter(a=>a.projeto_id===s.id&&a.status==="done").length;return`<div class="pj-side ${s.id===h?"on":""}" data-pid="${s.id}">
      <div class="pj-side-dot" style="background:${s.cor||"#C5F82A"}"></div>
      <div class="pj-side-body">
        <div class="pj-side-name">${v(s.nome)}</div>
        <div class="pj-side-meta">${o?v(o.empresa||o.nome):"Interno"} · ${d}/${n}</div>
      </div>
  ...
  </div>`,t.querySelector(".pj-sidebar-list").addEventListener("click",s=>{const o=s.target.closest(".pj-side");o&&(h=o.dataset.pid,X())}),R()}function R(){const t=j.find(a=>a.id===h);if(!t)return;const i=S.find(a=>a.id===t.cliente_id),s=$.filter(a=>a.projeto_id===t.id),o=D(t),n=`<div class="pj-head">
  ...
  </div>`,d=I==="lista"?dt(s,o):rt(s,o);document.getElementById("pj-main").innerHTML=n+d,document.getElementById("pj-edit").addEventListener("click",()=>G(t)),document.getElementById("pj-del").addEventListener("click",()=>ut(t)),document.getElementById("pj-add-tar").addEventListener("click",()=>{var a;return O({projeto_id:t.id,status:((a=o[0])==null?void 0:a.k)||"todo"})}),pt()}function D(t){const i=t==null?void 0:t.etapas;return Array.isArray(i)&&i.length?i:q}function rt(t,i){return`<div class="pj-kanban">${i.map(o=>{const n=t.filter(a=>a.status===o.k),d=n.length?n.map(a=>lt(a)).join(""):'<div class="pj-empty">—</div>';return`<div class="pj-col" data-status="${o.k}">
      <div class="pj-col-head">
        <span class="pj-col-title" style="color:${o.cor}">
          <span class="pj-col-bullet" style="background:${o.cor}"></span>${v(o.l)}
        </span>
        <span class="pj-col-count">${n.length}</span>
      </div>
      <div class="pj-col-cards" data-drop="${o.k}">${d}</div>
    </div>`}).join("")}</div>`}function dt(t,i){if(!t.length)return'<div class="empty" style="padding:40px">Sem tarefas neste projeto.</div>';const s=i.map(d=>d.k);return`<div class="tw" style="margin-top:14px">
    <table>
      <thead><tr><th>Status</th><th>Tarefa</th><th>Prio</th><th>Prazo</th><th></th></tr></thead>
      <tbody>${[...t].sort((d,a)=>s.indexOf(d.status)-s.indexOf(a.status)||(d.prazo||"").localeCompare(a.prazo||"")).map(d=>{const a=B.filter(c=>c.tarefa_id===d.id),e=a.filter(c=>c.feito).length;return`<tr class="pj-row" data-tid="${d.id}">
      <td>${ct(d.status,i)}</td>
  ...
  </div>`}function lt(t){const i=B.filter(n=>n.tarefa_id===t.id),s=i.filter(n=>n.feito).length,o=T[t.prioridade]||T.media;return`<div class="pj-card" draggable="true" data-tid="${t.id}">
    ...
    <div class="pj-card-foot">
      ${J(t.prazo)}
      ${i.length?`<span class="pj-sub-chip">${s}/${i.length}</span>`:""}
    </div>
  </div>`}function ct(t,i){const s=(i||q).find(o=>o.k===t)||q[0];return`<span class="pj-pill" style="background:${s.cor}22;color:${s.cor}">${v(s.l)}</span>`}function Y(t){const i=T[t]||T.media;return`<span class="pj-pill" style="background:${i.cor}22;color:${i.cor}">${i.l}</span>`}function J(t){if(!t)return"";const i=new Date(t+"T00:00:00"),s=new Date;s.setHours(0,0,0,0);const o=Math.round((i-s)/864e5);let n="pj-prazo";return o<0?n+=" late":o<=2&&(n+=" soon"),`<span class="${n}">${N(t)}</span>`}function pt(){const t=document.getElementById("pj-main");if(!t)return;t.addEventListener("click",s=>{const o=s.target.closest(".pj-card"),n=s.target.closest(".pj-row"),d=s.target.closest(".del-tar");if(d){s.stopPropagation();const a=$.find(e=>e.id===d.dataset.tid);a&&confirm(`Excluir "${a.titulo}"?`)&&tt(a.id);return}if(o){const a=$.find(e=>e.id===o.dataset.tid);a&&O(a)}else if(n){const a=$.find(e=>e.id===n.dataset.tid);a&&O(a)}});let i=null;t.querySelectorAll(".pj-card").forEach(s=>{s.addEventListener("dragstart",o=>{i=s.dataset.tid,s.classList.add("drag"),o.dataTransfer.effectAllowed="move"}),s.addEventListener("dragend",()=>{s.classList.remove("drag"),t.querySelectorAll(".pj-col-cards.over").forEach(o=>o.classList.remove("over"))})}),t.querySelectorAll(".pj-col-cards").forEach(s=>{s.addEventListener("dragover",o=>{o.preventDefault(),s.classList.add("over")}),s.addEventListener("dragleave",()=>s.classList.remove("over")),s.addEventListener("drop",async o=>{if(o.preventDefault(),s.classList.remove("over"),!i)return;const n=s.dataset.drop,d=$.find(e=>e.id===i);if(!d||d.status===n)return;if(n==="done"){await Z(d,()=>R());return}d.status=n;const{error:a}=await g.from("tarefas").update({status:n,atualizado_em:new Date().toISOString()}).eq("id",i);if(a){f("Erro: "+a.message,"err");return}R()})})}function G(t={}){const i='<option value="">— sem cliente (interno) —</option>'+et(t.cliente_id).map(a=>`<option value="${a.id}"${t.cliente_id===a.id?" selected":""}>${v(a.empresa||a.nome)}</option>`).join(""),s=P.map(a=>`<span class="pj-cor-opt${(t.cor||P[0])===a?" on":""}" data-cor="${a}" style="background:${a}"></span>`).join("");let o=Array.isArray(t.etapas)&&t.etapas.length?JSON.parse(JSON.stringify(t.etapas)):JSON.parse(JSON.stringify(q));z(t.id?"Editar projeto":"Novo projeto",`
  ...
  `);let n=t.cor||P[0];document.getElementById("pj-cor-row").addEventListener("click",a=>{const e=a.target.closest(".pj-cor-opt");e&&(n=e.dataset.cor,document.querySelectorAll(".pj-cor-opt").forEach(c=>c.classList.toggle("on",c.dataset.cor===n)))});const d=()=>{document.getElementById("pj-etapas-list").innerHTML=o.map((a,e)=>`
  ...
  `);const a=r=>{x(),i&&i()},e=()=>document.getElementById("cn-apelido").value.trim()||t.titulo,c=()=>document.getElementById("cn-incluir").checked;document.getElementById("cn-skip").addEventListener("click",async()=>{await g.from("tarefas").update({apelido_cliente:e(),notificar_cliente:!1}).eq("id",t.id),f("Concluída — cliente não vai ser avisado"),a()}),document.getElementById("cn-save").addEventListener("click",async()=>{const r=e(),l=c();await g.from("tarefas").update({apelido_cliente:r,notificar_cliente:l}).eq("id",t.id),f(l?"No resumão das 18h":"Concluída — sem aviso"),a()}),document.getElementById("cn-now").addEventListener("click",async()=>{const r=document.getElementById("cn-now");r.disabled=!0,r.textContent="Enviando…";try{const l=e();await g.from("tarefas").update({apelido_cliente:l,notificar_cliente:!0}).eq("id",t.id);const p=j.find(u=>u.id===t.projeto_id),b=p&&p.jid_grupo||n.whatsapp;await M("digest-cliente",{bloco_id:b,apelidos:{[t.id]:l}}),f(`Enviado pra ${n.empresa||n.nome} ✓`),a()}catch(l){r.disabled=!1,r.textContent="Avisar agora",f("Erro ao enviar: "+l.message,"err")}})}function O(t={}){const i=!t.id,s=j.find(e=>e.id===(t.projeto_id||h)),o=D(s),n=o.map(e=>`<option value="${e.k}"${(t.status||o[0].k)===e.k?" selected":""}>${v(e.l)}</option>`).join(""),d=H.map(e=>`<option value="${e.k}"${(t.prioridade||"media")===e.k?" selected":""}>${e.l}</option>`).join(""),a=i?[]:B.filter(e=>e.tarefa_id===t.id);z(i?"Nova tarefa":"Editar tarefa",`
    ...
    <div class="frow" style="margin-bottom:11px">
      <div class="fg"><label class="fl">Etapa</label><select class="fsl" id="tr-status">${n}</select></div>
  ...
  `),document.getElementById("tr-sub-add").addEventListener("click",()=>{const e=document.getElementById("tr-sub-new"),c=e.value.trim();if(!c)return;const r="new-"+Math.random().toString(36).slice(2);document.getElementById("tr-subs").insertAdjacentHTML("beforeend",Q(r,c,!1)),e.value="",e.focus()}),document.getElementById("tr-sub-new").addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),document.getElementById("tr-sub-add").click())}),document.getElementById("tr-subs").addEventListener("click",e=>{const c=e.target.closest(".pj-sub-del");c&&c.closest(".pj-sub-row").remove()}),document.getElementById("tr-cancel").addEventListener("click",x),i||document.getElementById("tr-del").addEventListener("click",async()=>{confirm(`Excluir "${t.titulo}"?`)&&(await tt(t.id),x())}),document.getElementById("tr-save").addEventListener("click",async()=>{const e={projeto_id:t.projeto_id||h,titulo:document.getElementById("tr-titulo").value.trim(),descricao:document.getElementById("tr-desc").value.trim()||null,status:document.getElementById("tr-status").value,prioridade:document.getElementById("tr-prio").value,prazo:document.getElementById("tr-prazo").value||null,apelido_cliente:document.getElementById("tr-apelido").value.trim()||null,notificar_cliente:document.getElementById("tr-notif").checked,atualizado_em:new Date().toISOString()};if(!e.titulo)return f("Título é obrigatório","err");let c=t.id;if(i){const{data:u,error:m}=await g.from("tarefas").insert(e).select().single();if(m)return f("Erro: "+m.message,"err");c=u.id}else{const{error:u}=await g.from("tarefas").update(e).eq("id",c);if(u)return f("Erro: "+u.message,"err")}const r=[...document.querySelectorAll(".pj-sub-row")],l=i?[]:B.filter(u=>u.tarefa_id===c),p=new Set;let b=0;for(const u of r){const m=u.dataset.sid,y=u.querySelector(".pj-sub-text").value.trim(),k=u.querySelector(".pj-sub-cb").checked;y&&(m.startsWith("new-")?await g.from("tarefa_subtasks").insert({tarefa_id:c,texto:y,feito:k,ordem:b++}):(p.add(m),await g.from("tarefa_subtasks").update({texto:y,feito:k,ordem:b++}).eq("id",m)))}for(const u of l)p.has(u.id)||await g.from("tarefa_subtasks").delete().eq("id",u.id);x(),f(i?"Tarefa criada":"Tarefa salva"),_()})}function Q(t,i,s){return`<div class="pj-sub-row" data-sid="${t}">
  ...
  </div>`}async function tt(t){const{error:i}=await g.from("tarefas").delete().eq("id",t);if(i)return f("Erro: "+i.message,"err");f("Tarefa excluída"),_()}function C(t={}){const i=!t.id,s='<option value="">— sem cliente —</option>'+et(t.cliente_id).map(r=>`<option value="${r.id}"${t.cliente_id===r.id?" selected":""}>${v(r.empresa||r.nome)}</option>`).join(""),o='<option value="">— escolha um projeto —</option>'+j.map(r=>`<option value="${r.id}"${t.projeto_id===r.id?" selected":""}>${v(r.nome)}</option>`).join(""),n=t.cadencia||"semanal",d=t.dias_semana||[1],a=Array.isArray(t.tarefas)?JSON.parse(JSON.stringify(t.tarefas)):[],e=U.map(r=>`<label class="pj-dia-chip ${d.includes(r.k)?"on":""}" data-d="${r.k}"><input type="checkbox" ${d.includes(r.k)?"checked":""} style="display:none">${r.l}</label>`).join("");z(i?"Nova rotina":"Editar rotina",`
    <div class="fg" style="margin-bottom:11px"><label class="fl">Nome *</label>
    ...
    <div class="fg" style="margin-bottom:11px">
      ...
      <div class="pj-cad-row">
        <label class="pj-cad-opt ${n==="diaria"?"on":""}"><input type="radio" name="rt-cad" value="diaria" ${n==="diaria"?"checked":""}>Todo dia</label>
        <label class="pj-cad-opt ${n==="semanal"?"on":""}"><input type="radio" name="rt-cad" value="semanal" ${n==="semanal"?"checked":""}>Semanal</label>
        <label class="pj-cad-opt ${n==="mensal"?"on":""}"><input type="radio" name="rt-cad" value="mensal" ${n==="mensal"?"checked":""}>Mensal</label>
      </div>
    </div>
    <div class="fg" id="rt-dias-wrap" style="margin-bottom:11px;${n==="semanal"?"":"display:none"}">
      <label class="fl">Dias da semana</label>
      <div class="pj-dias-row" id="rt-dias">${e}</div>
    </div>
    <div class="fg" id="rt-mes-wrap" style="margin-bottom:11px;${n==="mensal"?"":"display:none"}">

import{s as wt,d as _,t as u,o as It,f as U}from"./index-Ua5pMpeP.js";const f=e=>String(e??"").replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]);let y=[],T=null;const ot="teleprompter_prefs";function h(){try{return JSON.parse(localStorage.getItem(ot)||"{}")}catch{return{}}}function C(e){localStorage.setItem(ot,JSON.stringify(e))}function at(e,r){const a=String(e||"").trim().split(/\s+/).filter(Boolean).length;return Math.max(1,Math.round(a/r*60))}function it(e){if(e<60)return`${e}s`;const r=Math.floor(e/60),a=e%60;return a?`${r}min ${a}s`:`${r}min`}async function Q(){const e=document.getElementById("content");document.getElementById("tbacts").innerHTML=`
    <button class="btn bp" id="tp-new">+ Novo roteiro</button>`,e.innerHTML='<div class="empty">Carregando roteiros...</div>';const{data:r,error:a}=await wt("teleprompter_scripts",{order:{column:"atualizado_em",ascending:!1}});if(a){e.innerHTML=`
      ...
      </div>`,document.getElementById("tp-new").disabled=!0;return}if(y=r||[],document.getElementById("tp-new").addEventListener("click",()=>X()),!y.length){e.innerHTML=`
      <div class="empty" style="padding:60px 20px;text-align:center">
    ...
    <div style="margin-bottom:14px;font-size:12px;color:var(--text-3)">
      ${y.length} roteiro${y.length===1?"":"s"} · dica: rotaciona o celular pro modo paisagem antes de executar
    </div>
    <div id="tp-list" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px"></div>`,document.getElementById("tp-list").innerHTML=y.map(kt).join(""),document.querySelectorAll(".tp-play").forEach(o=>o.addEventListener("click",()=>{const s=y.find(d=>d.id===o.dataset.id);s&&Lt(s)})),document.querySelectorAll(".tp-edit").forEach(o=>o.addEventListener("click",()=>{const s=y.find(d=>d.id===o.dataset.id);s&&X(s)})),document.querySelectorAll(".tp-del").forEach(o=>o.addEventListener("click",async()=>{const s=y.find(v=>v.id===o.dataset.id);if(!s||!confirm(`Excluir roteiro "${s.titulo}"?`))return;const{error:d}=await _.from("teleprompter_scripts").delete().eq("id",s.id);if(d){u("Erro: "+d.message,"err");return}u("Roteiro excluido"),Q()}))}function kt(e){const r=at(e.conteudo,e.velocidade_wpm||150),a=(e.conteudo||"").replace(/\s+/g," ").slice(0,100),o=(e.tags||[]).map(s=>`<span style="font-size:10px;background:rgba(255,255,255,.04);color:var(--text-3);padding:2px 7px;border-radius:10px;border:1px solid var(--line)">${f(s)}</span>`).join("");return`
    ...
    </div>`}function X(e={}){const r=!e.id,a=(e.tags||[]).join(", "),o=e.conteudo||"",s=e.velocidade_wpm||150,d=e.tamanho_fonte||48;It(r?"Novo roteiro":"Editar roteiro",`
    ...
    <div class="frow" style="margin-bottom:11px">
      <div class="fg">
        ...
        <input class="fi" type="number" id="tp-wpm" value="${s}" min="60" max="300" step="10">
      ...
      <div class="fg">
        ...
        <input class="fi" type="number" id="tp-fonte" value="${d}" min="24" max="120" step="4">
  ...
  `);const v=()=>{const x=document.getElementById("tp-conteudo").value,g=Number(document.getElementById("tp-wpm").value)||150,E=x.trim().split(/\s+/).filter(Boolean).length,i=at(x,g);document.getElementById("tp-stats").textContent=`${E} palavras · duracao estimada ~${it(i)}`};document.getElementById("tp-conteudo").addEventListener("input",v),document.getElementById("tp-wpm").addEventListener("input",v),v(),document.getElementById("tp-cancel").addEventListener("click",U),r||document.getElementById("tp-del-modal").addEventListener("click",async()=>{confirm(`Excluir "${e.titulo}"?`)&&(await _.from("teleprompter_scripts").delete().eq("id",e.id),U(),u("Roteiro excluido"),Q())}),document.getElementById("tp-save").addEventListener("click",async()=>{const x=document.getElementById("tp-titulo").value.trim(),g=document.getElementById("tp-conteudo").value.trim();if(!x)return u("Titulo obrigatorio","err");if(!g)return u("Roteiro vazio","err");const E={titulo:x,conteudo:g,velocidade_wpm:Number(document.getElementById("tp-wpm").value)||150,tamanho_fonte:Number(document.getElementById("tp-fonte").value)||48,tags:document.getElementById("tp-tags").value.split(",").map(l=>l.trim()).filter(Boolean),atualizado_em:new Date().toISOString()},i=r?_.from("teleprompter_scripts").insert(E).select().single():_.from("teleprompter_scripts").update(E).eq("id",e.id).select().single(),{error:w}=await i;if(w)return u("Erro: "+w.message,"err");u(r?"Roteiro criado":"Atualizado"),U(),Q()})}async function Lt(e){const r=h();let a=r.wpm??e.velocidade_wpm??150,o=r.fonte??e.tamanho_fonte??48,s=!!r.mirror,d=!1,v=0,x=0,g=0,E=0,i=null,w=!1,l=null,F=[],W=0,$=0,I=[],k=h().audioDeviceId||null,M=!!h().audioModoExterno;const b=document.createElement("div");b.id="tp-player-ov",b.setAttribute("role","application"),b.innerHTML=`
    ...
    </style>`,document.body.appendChild(b);const L=document.getElementById("tp-scroll"),rt=b.querySelector(".tp-player-inner"),P=document.getElementById("tp-play"),st=document.getElementById("tp-close"),Y=document.getElementById("tp-mirror"),lt=document.getElementById("tp-speed-lbl"),dt=document.getElementById("tp-texto"),S=document.getElementById("tp-controls"),J=document.getElementById("tp-hint");async function ct(){if("wakeLock"in navigator)try{T=await navigator.wakeLock.request("screen")}catch{}}async function K(){try{await(T==null?void 0:T.release()),T=null}catch{}}function H(){S.classList.remove("hidden"),clearTimeout(E),E=setTimeout(()=>{d&&S.classList.add("hidden")},2800)}function pt(){const t=a/10,n=o*1.35;return t*n/60}function V(t){if(!d)return;g||(g=t);const n=(t-g)/1e3;g=t,x+=pt()*n,L.scrollTop=x,L.scrollTop+L.clientHeight>=L.scrollHeight-4&&(A(),S.classList.remove("hidden")),v=requestAnimationFrame(V)}function G(){d||(d=!0,g=0,P.textContent="⏸",J.classList.add("hidden"),H(),ct(),v=requestAnimationFrame(V))}function A(){d=!1,P.textContent="▶",S.classList.remove("hidden"),cancelAnimationFrame(v),K()}function N(){d?A():G()}function Z(){if(A(),l&&l.state!=="inactive")try{l.stop()}catch{}$&&clearInterval($),i&&(i.getTracks().forEach(t=>t.stop()),i=null),K(),b.remove(),_.from("teleprompter_scripts").update({vezes_usado:(e.vezes_usado||0)+1,ultimo_uso:new Date().toISOString()}).eq("id",e.id).then(()=>{},()=>{}),document.removeEventListener("keydown",et),document.fullscreenElement&&document.exitFullscreen().catch(()=>{})}function mt(){if(M){const n={autoGainControl:!1,noiseSuppression:!1,echoCancellation:!1,sampleRate:48e3,channelCount:1};return k&&(n.deviceId={exact:k}),n}const t={autoGainControl:!1,noiseSuppression:!0,echoCancellation:!0,sampleRate:48e3,channelCount:1};return k&&(t.deviceId={exact:k}),t}async function tt(){try{i=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:1920},height:{ideal:1080},frameRate:{ideal:30}},audio:mt()});const t=document.getElementById("tp-cam");t.srcObject=i,w=!0,b.classList.add("tp-cam-on"),document.getElementById("tp-cam-btn").textContent="🎥",document.getElementById("tp-cam-btn").classList.add("tp-btn-on"),document.getElementById("tp-rec-btn").style.display="",document.getElementById("tp-mic-btn").style.display="";try{I=(await navigator.mediaDevices.enumerateDevices()).filter(m=>m.kind==="audioinput"),ut()}catch{}u("Camera ativa. Texto rola por cima do video.")}catch(t){console.error("[tp-cam]",t),u("Nao consegui acessar a camera: "+(t.message||t.name),"err")}}function ut(){var B,j,nt;const t=document.getElementById("tp-mic-chip");if(!t)return;const n=(B=i==null?void 0:i.getAudioTracks)==null?void 0:B.call(i)[0],c=(((j=n==null?void 0:n.getSettings)==null?void 0:j.call(n))||{}).deviceId||k,p=I.find(Et=>Et.deviceId===c),z=((nt=p==null?void 0:p.label)==null?void 0:nt.replace(/\(.*\)$/,"").trim())||"Mic";t.innerHTML=`🎙 <strong style="color:#fff">${f(z.slice(0,26))}</strong>${M?' · <span style="color:#C5F82A">externo</span>':""}`,t.style.display=""}async function ft(){if(!I.length)return;const t=I.map((c,p)=>`${p+1}. ${c.label||"Mic "+(p+1)}`).join(`
`),n=prompt(`Escolhe o mic (1-${I.length}):

${t}

Ou escreve "externo" pra alternar modo mic externo (sem processing).`);if(!n)return;if(/^ext/i.test(n))M=!M,C({...h(),audioModoExterno:M});else{const c=parseInt(n,10);if(!(c>=1&&c<=I.length))return;k=I[c-1].deviceId,C({...h(),audioDeviceId:k})}l&&l.state==="recording"&&O(),i==null||i.getTracks().forEach(c=>c.stop()),i=null,await tt()}function gt(){l&&l.state!=="inactive"&&O(),i&&(i.getTracks().forEach(t=>t.stop()),i=null),document.getElementById("tp-cam").srcObject=null,w=!1,b.classList.remove("tp-cam-on"),document.getElementById("tp-cam-btn").classList.remove("tp-btn-on"),document.getElementById("tp-rec-btn").style.display="none"}function vt(){const t=["video/mp4;codecs=avc1,mp4a","video/mp4","video/webm;codecs=vp9,opus","video/webm;codecs=vp8,opus","video/webm"];for(const n of t)if(MediaRecorder.isTypeSupported&&MediaRecorder.isTypeSupported(n))return n;return""}function bt(){if(!i)return;F=[];const t=vt(),n={videoBitsPerSecond:4e6,audioBitsPerSecond:192e3};t&&(n.mimeType=t);try{l=new MediaRecorder(i,n)}catch{try{l=new MediaRecorder(i,t?{mimeType:t,videoBitsPerSecond:4e6}:void 0)}catch{u("Gravacao nao suportada nesse navegador","err");return}}l.ondataavailable=m=>{m.data&&m.data.size&&F.push(m.data)},l.onstop=()=>{const m=l.mimeType||t||"video/webm",c=new Blob(F,{type:m}),p=URL.createObjectURL(c),z=m.includes("mp4")?"mp4":"webm",B=`${e.titulo.replace(/[^\w-]+/g,"_").slice(0,40)}_${new Date().toISOString().slice(0,16).replace(/[:T]/g,"-")}.${z}`;yt(p,B,c.size)},l.start(1e3),W=Date.now(),document.getElementById("tp-rec-badge").style.display="flex",document.getElementById("tp-rec-btn").textContent="⏹",document.getElementById("tp-rec-btn").classList.add("on"),$=setInterval(()=>{const m=Math.floor((Date.now()-W)/1e3),c=Math.floor(m/60),p=m%60;document.getElementById("tp-rec-time").textContent=`${c}:${String(p).padStart(2,"0")}`},500),d||G()}function O(){if(!(!l||l.state==="inactive")){try{l.stop()}catch{}clearInterval($),$=0,document.getElementById("tp-rec-badge").style.display="none",document.getElementById("tp-rec-btn").textContent="🔴",document.getElementById("tp-rec-btn").classList.remove("on"),d&&A()}}function xt(){w&&(l&&l.state==="recording"?O():bt())}function yt(t,n,m){const c=(m/1024/1024).toFixed(1),p=document.createElement("div");p.style.cssText=`
    ...
    `,p.innerHTML=`
      ...
      <div style="font-size:12px;color:var(--text-3)">${f(n)} · ${c} MB</div>
    ...
    `,document.body.appendChild(p),p.querySelector("#tp-descartar").addEventListener("click",()=>{URL.revokeObjectURL(t),p.remove()}),p.querySelector("#tp-share").addEventListener("click",async()=>{if(!navigator.share){u("Compartilhamento nao suportado, use Baixar","warn");return}try{const B=await(await fetch(t)).blob(),j=new File([B],n,{type:B.type});await navigator.share({files:[j],title:e.titulo})}catch(z){z.name!=="AbortError"&&u("Erro ao compartilhar","err")}})}function et(t){t.key==="Escape"&&(Z(),t.preventDefault()),t.key===" "&&(N(),t.preventDefault()),t.key==="ArrowUp"&&(o=Math.min(120,o+4),q()),t.key==="ArrowDown"&&(o=Math.max(24,o-4),q()),t.key==="ArrowRight"&&(a=Math.min(300,a+10),R()),t.key==="ArrowLeft"&&(a=Math.max(60,a-10),R())}function q(){dt.style.fontSize=o+"px",C({...h(),fonte:o})}function R(){lt.textContent=a+" wpm",C({...h(),wpm:a})}function ht(){rt.style.transform=s?"scaleX(-1)":"none",Y.classList.toggle("tp-btn-on",s),C({...h(),mirror:s})}P.addEventListener("click",N),st.addEventListener("click",Z),Y.addEventListener("click",()=>{s=!s,ht()}),document.getElementById("tp-cam-btn").addEventListener("click",()=>{w?gt():tt()}),document.getElementById("tp-rec-btn").addEventListener("click",xt),document.getElementById("tp-mic-btn").addEventListener("click",ft),document.getElementById("tp-speed-up").addEventListener("click",()=>{a=Math.min(300,a+10),R()}),document.getElementById("tp-speed-down").addEventListener("click",()=>{a=Math.max(60,a-10),R()}),document.getElementById("tp-font-up").addEventListener("click",()=>{o=Math.min(120,o+4),q()}),document.getElementById("tp-font-down").addEventListener("click",()=>{o=Math.max(24,o-4),q()}),L.addEventListener("click",t=>{t.target.closest(".tp-btn")||t.target.closest(".tp-btn-group")||N()}),L.addEventListener("touchstart",H,{passive:!0}),L.addEventListener("mousemove",H),document.addEventListener("keydown",et),setTimeout(()=>J.classList.add("hidden"),4e3);try{await document.documentElement.requestFullscreen()}catch{}const D=document.createElement("div");D.style.cssText=`
  ...
  `,b.appendChild(D);for(const t of["3","2","1","GO"])D.textContent=t,await new Promise(n=>setTimeout(n,t==="GO"?500:800));D.remove(),G()}export{Q as render};

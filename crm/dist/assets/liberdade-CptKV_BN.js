import{d as w,t as x,c as y,o as gt,f as U,M as $t,h as Bt,j as Lt}from"./index-DxIO5JPM.js";let k=null,_=[],B=[],et=[],nt=[],xt=[],yt=[],lt=[],_t=[],ot=null,d={metaId:null,meses:60,taxa:14.25,fases:[{inicio:0,aporte:2e3}]};const it=["#C5F82A","#4A9EFF","#A78BFA","#F5A623","#EC4899","#34D399","#FF6B35","#06B6D4"],bt=["🚀","💻","✈️","🏠","🚗","💍","📚","🎓","🎯","💰","🏖️","📱","🎸","🎮"],zt=360*60*1e3;let E={open:!1,msgs:[],loading:!1};async function q(){var a;const t=document.getElementById("content");if(t.innerHTML='<div class="empty">Carregando…</div>',await K(),await ct(),Rt(),!d.metaId||!_.find(s=>s.id===d.metaId)){const s=_.find(o=>o.principal)||_[0];d.metaId=(s==null?void 0:s.id)||null}(a=d.fases)!=null&&a.length||(d.fases=[{inicio:0,aporte:2e3}]),d.fases[0].aporte=Math.max(d.fases[0].aporte,J()),d.taxa=Number(k.selic_aa)||14.25,t.innerHTML=Yt(),ne(t),Et(t);const e=t.querySelector(".lib-sim-chart");e&&Mt(e)}async function K(){const[t,e,a,s,o,n,l,i,c]=await Promise.all([w.from("config_fin").select("*").eq("id","main").maybeSingle(),w.from("metas_fin").select("*").neq("status","arquivada").order("principal",{ascending:!1}).order("ordem"),w.from("aportes_fin").select("*").order("data",{ascending:!1}),w.from("faturamento").select("*").order("ano",{ascending:!1}).order("mes",{ascending:!1}),w.from("despesas").select("*").order("data",{ascending:!1}),w.from("despesas_recorrentes").select("*").order("criado_em",{ascending:!1}),w.from("receitas_recorrentes").select("*").order("criado_em",{ascending:!1}),w.from("caixinhas").select("*").eq("ativa",!0).order("ordem"),w.from("caixinhas_mov").select("*").order("data",{ascending:!1})]);t.error&&x("config_fin: "+t.error.message,"err"),e.error&&x("metas_fin: "+e.error.message,"err"),a.error&&x("aportes_fin: "+a.error.message,"err"),k=t.data||{id:"main",meta_brl:1e5,saldo_atual_brl:0,selic_aa:14.25,aporte_pct_faturamento:20},_=e.data||[],B=a.data||[],et=s.data||[],nt=o&&!o.error?o.data||[]:[],xt=n&&!n.error?n.data||[]:[],yt=l&&!l.error?l.data||[]:[],lt=i&&!i.error?i.data||[]:[],_t=c&&!c.error?c.data||[]:[]}async function ct({manual:t=!1}={}){var e;try{const a=await fetch("https://api.bcb.gov.br/dados/serie/bcdata.sgs.432/dados/ultimos/1?formato=json");if(!a.ok)return t&&x("BCB respondeu "+a.status,"err"),!1;const s=await a.json(),o=Number((e=s==null?void 0:s[0])==null?void 0:e.valor);if(!isFinite(o)||o<=0)return t&&x("Resposta inválida do BCB","err"),!1;const n=Number(k.selic_aa),l=Math.abs(o-n)>.01,i=new Date().toISOString();return k.selic_aa=o,k.atualizado_em=i,await w.from("config_fin").upsert({id:"main",selic_aa:o,atualizado_em:i}),l?x(`Selic atualizada: ${o.toFixed(2)}% a.a.`):t&&x(`Selic confirmada: ${o.toFixed(2)}% a.a.`),Dt(),!0}catch{return t&&x("Sem conexão com o BCB","err"),!1}}function Rt(){ot&&clearInterval(ot),ot=setInterval(()=>{ct()},zt)}function Dt(){const t=document.querySelector(".lib-chip-selic");t&&(t.innerHTML=At())}function At(){const t=Number(k.selic_aa).toFixed(2),e=k.atualizado_em?Ot(k.atualizado_em):"não verificada";return`<b>Selic</b> ${t}% a.a. <small>BCB · ${e}</small>
    <button class="lib-selic-refresh" id="lib-selic-refresh" title="Atualizar agora">↻</button>`}function Ot(t){const e=Math.floor((Date.now()-new Date(t).getTime())/1e3);return e<60?"agora":e<3600?`há ${Math.floor(e/60)}min`:e<86400?`há ${Math.floor(e/3600)}h`:e<86400*30?`há ${Math.floor(e/86400)}d`:new Date(t).toLocaleDateString("pt-BR")}function wt(t,e){if(!(t!=null&&t.length))return 0;const a=[...t].sort((o,n)=>o.inicio-n.inicio);let s=0;for(const o of a)o.inicio<=e&&(s=Number(o.aporte)||0);return s}function dt({pv:t,fases:e,taxaAA:a,meses:s}){const o=Math.pow(1+a/100,.08333333333333333)-1,n=[];let l=t,i=t;for(let c=0;c<=s;c++){if(c>0){const p=wt(e,c-1);l=l*(1+o)+p,i+=p}n.push({mes:c,saldo:l,aportado:i,juros:l-i})}return n}function at({pv:t,fases:e,taxaAA:a,meta:s}){if(t>=s)return 0;const o=Math.pow(1+a/100,1/12)-1;let n=t,l=0;for(;l<1200;){const i=wt(e,l);if(n=n*(1+o)+i,l++,n>=s)return l;if(i===0&&n<=t*1.0001&&l>12)return null}return null}function kt(t,e){return et.filter(a=>a.ano===t&&a.mes===e).reduce((a,s)=>a+Number(s.valor||0),0)}function ut(){const t=new Date;let e=0,a=0;for(let s=1;s<=6;s++){const o=new Date(t.getFullYear(),t.getMonth()-s,1),n=kt(o.getFullYear(),o.getMonth()+1);n>0&&(e+=n,a++)}return a?e/a:0}function mt(){const t=new Date;return kt(t.getFullYear(),t.getMonth()+1)}function J(){const t=Number((k==null?void 0:k.aporte_pct_faturamento)||20)/100,e=ut()||mt();return Math.max(0,Math.round(e*t))}function Q(t){const e=B.filter(a=>a.meta_id===t.id).reduce((a,s)=>a+Number(s.valor||0),0);return Number(t.valor_inicial||0)+e}function rt(t){return B.filter(e=>e.meta_id===t.id)}function Tt(t){const e=new Date().toISOString().slice(0,7);return B.filter(a=>(t?a.meta_id===t:!0)&&(a.data||"").startsWith(e)).reduce((a,s)=>a+Number(s.valor||0),0)}function Pt(t){const e=J()||1;let a=0;const s=new Date;for(let o=0;o<36;o++){const l=new Date(s.getFullYear(),s.getMonth()-o,1).toISOString().slice(0,7),i=B.filter(c=>(t?c.meta_id===t:!0)&&(c.data||"").startsWith(l)).reduce((c,p)=>c+Number(p.valor||0),0);if(i>=e&&i>0)a++;else break}return a}function Ht(){var N;const t=_.find(r=>r.principal)||_[0],e=_.find(r=>r.id===d.metaId)||t,a=new Date().toISOString().slice(0,10),s=new Date,o=_.map(r=>{const f=Q(r);return{nome:r.nome,alvo:Number(r.valor_alvo),saldo_atual:f,progresso_pct:+(f/Number(r.valor_alvo)*100).toFixed(1),principal:r.principal===!0,prazo_meses:r.prazo_meses||null}}),n=B.slice(0,12).map(r=>{var f;return{data:r.data,valor:Number(r.valor),meta:((f=_.find(m=>m.id===r.meta_id))==null?void 0:f.nome)||null,fonte:r.fonte,obs:r.observacao}}),l=[];for(let r=0;r<=11;r++){const f=new Date(s.getFullYear(),s.getMonth()-r,1),m=f.getMonth()+1,v=f.getFullYear(),F=et.filter(M=>M.mes===m&&M.ano===v).reduce((M,C)=>M+Number(C.valor||0),0);l.push({mes:`${v}-${String(m).padStart(2,"0")}`,valor:+F.toFixed(2)})}const i=et.filter(r=>new Date(r.ano,r.mes-1,1)>new Date(s.getFullYear(),s.getMonth(),1)).map(r=>({mes:`${r.ano}-${String(r.mes).padStart(2,"0")}`,valor:+Number(r.valor).toFixed(2),desc:r.descricao||null})),c=a,p=[];for(let r=0;r<=11;r++){const f=new Date(s.getFullYear(),s.getMonth()-r,1);p.push(`${f.getFullYear()}-${String(f.getMonth()+1).padStart(2,"0")}`)}const b=nt.filter(r=>p.some(f=>(r.data||"").startsWith(f))&&(r.data||"")<=c),h={},g={};for(const r of b){const f=r.categoria||"outro";h[f]=(h[f]||0)+Number(r.valor||0);const m=(r.data||"").slice(0,7);g[m]=(g[m]||0)+Number(r.valor||0)}const A=Object.values(h).reduce((r,f)=>r+f,0);for(const r of Object.keys(g))g[r]=+g[r].toFixed(2);const u=nt.filter(r=>(r.data||"")>c).sort((r,f)=>(r.data||"").localeCompare(f.data||"")).map(r=>({data:r.data,descricao:r.descricao||"(sem descrição)",categoria:r.categoria||"outro",valor:Number(r.valor||0)})),$={};for(const r of u){const f=r.data.slice(0,7);$[f]||($[f]={total:0,qtd:0,itens:[]}),$[f].total+=r.valor,$[f].qtd++,$[f].itens.push(`${r.data}: ${r.descricao} (${r.categoria}) — R$${r.valor.toFixed(2)}`)}for(const r of Object.keys($))$[r].total=+$[r].total.toFixed(2);const j=u.reduce((r,f)=>r+f.valor,0);let T=0;if(u.length){const r=new Date(u[u.length-1].data);T=(r.getFullYear()-s.getFullYear())*12+(r.getMonth()-s.getMonth())}const V=xt.filter(r=>r.ativa).map(r=>({descricao:r.descricao,valor:Number(r.valor),categoria:r.categoria,dia:r.dia_vencimento||null})),P=V.reduce((r,f)=>r+f.valor,0),R=yt.filter(r=>r.ativa).map(r=>({descricao:r.descricao,valor:Number(r.valor)})),O=R.reduce((r,f)=>r+f.valor,0),I=Math.max(6,T),Y=+(P*I+j).toFixed(2),z=s.getMonth()+1,S=s.getFullYear(),G=lt.map(r=>{const f=`${S}-${String(z).padStart(2,"0")}`,m=_t.filter(F=>F.caixinha_id===r.id&&(F.data||"").startsWith(f)).reduce((F,M)=>F+Number(M.valor||0),0),v=r.tipo==="gasto"?Number(r.valor_mensal)-m:null;return{id:r.id,nome:r.nome,tipo:r.tipo,valor_mensal:Number(r.valor_mensal),icone:r.icone,cor:r.cor,usado_mes_atual:+m.toFixed(2),saldo_mes_atual:v!==null?+v.toFixed(2):null}});return{hoje:a,selic_aa:Number(k.selic_aa),selic_atualizada_em:k.atualizado_em,faturamento:{medio_6m:+ut().toFixed(2),mes_atual:+mt().toFixed(2),ultimos_12_meses:l,futuro_lancado:i,pct_alvo_aporte:Number(k.aporte_pct_faturamento),aporte_sugerido_mensal:J()},despesas_passadas:{total_ultimos_12_meses:+A.toFixed(2),media_mensal_12m:+(A/12).toFixed(2),por_categoria_12m:Object.fromEntries(Object.entries(h).map(([r,f])=>[r,+f.toFixed(2)])),por_mes_12m:g},despesas_futuras_agendadas:{total:+j.toFixed(2),qtd:u.length,horizonte_meses:T,ultima_data:((N=u[u.length-1])==null?void 0:N.data)||null,por_mes:$,lista_completa:u},contas_fixas_mensais:{total:+P.toFixed(2),lista:V,total_projetado_no_horizonte:+(P*I).toFixed(2),meses_projetados:I},receitas_recorrentes:{total_mensal:+O.toFixed(2),lista:R},compromisso_futuro_total:Y,metas:o,aportes:{total_geral:+B.reduce((r,f)=>r+Number(f.valor||0),0).toFixed(2),qtd_total:B.length,ultimos_12:n},caixinhas:G,simulador_atual:{meta_em_foco:e==null?void 0:e.nome,taxa_aa:d.taxa,prazo_meses:d.meses,fases:d.fases}}}async function Jt(t){var i,c,p,b;const e=Lt.OR_KEY;if(!e)throw new Error("Chave da IA não disponível. Faça login de novo.");const a=Ht(),s=`Você é um planejador financeiro pessoal direto, brasileiro, sem firula. Conversa com o usuário sobre as finanças dele dentro do CRM "Eleva Digital". Linguagem informal, tipo amigo que entende de finanças. Seja sincero quando os números não fecham.
...
Use IDs reais do contexto (caixinhas[].id). Use APENAS quando o usuário pedir explicitamente ou quando você tiver uma sugestão clara de reorganização. Não polua a resposta com muitos blocos — 1 a 3 por mensagem no máximo.`,o=["anthropic/claude-sonnet-4.6","anthropic/claude-haiku-4.5","meta-llama/llama-3.3-70b-instruct"],n=[{role:"system",content:s},...E.msgs.slice(-10).map(h=>({role:h.role,content:h.content})),{role:"user",content:t}];let l="";for(const h of o){const g=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify({model:h,messages:n,max_tokens:800,temperature:.6})});if(g.ok){const u=await g.json();return((b=(p=(c=(i=u==null?void 0:u.choices)==null?void 0:i[0])==null?void 0:c.message)==null?void 0:p.content)==null?void 0:b.trim())||"(sem resposta)"}if(g.status===404){l=`${h} indisponível`;continue}const A=await g.text().catch(()=>"");throw new Error(`OpenRouter ${g.status}: ${A.slice(0,200)}`)}throw new Error(l||"Nenhum modelo disponível")}async function ft(t){if(!(!t.trim()||E.loading)){E.msgs.push({role:"user",content:t.trim()}),E.loading=!0,D();try{const e=await Jt(t.trim());E.msgs.push({role:"assistant",content:e})}catch(e){E.msgs.push({role:"assistant",content:"⚠️ Erro: "+e.message})}finally{E.loading=!1,D()}}}function D(){var s;const t=document.getElementById("lib-chat");if(!t)return;const e=((s=St().match(/<div id="lib-chat"[\s\S]*<\/div>/))==null?void 0:s[0])||"";t.outerHTML=e,Et(document.getElementById("content"));const a=document.getElementById("lib-chat-body");a&&(a.scrollTop=a.scrollHeight)}function Et(t){var s,o,n;(s=t.querySelector("#lib-fab"))==null||s.addEventListener("click",()=>{E.open=!0,D(),setTimeout(()=>{var l;return(l=document.getElementById("lib-chat-text"))==null?void 0:l.focus()},250)}),(o=t.querySelector("#lib-chat-close"))==null||o.addEventListener("click",()=>{E.open=!1,D()}),(n=t.querySelector("#lib-chat-clear"))==null||n.addEventListener("click",()=>{confirm("Limpar conversa?")&&(E.msgs=[],D())});const e=t.querySelector("#lib-chat-input"),a=t.querySelector("#lib-chat-text");e==null||e.addEventListener("submit",l=>{l.preventDefault();const i=a.value;a.value="",a.style.height="auto",ft(i)}),a==null||a.addEventListener("keydown",l=>{l.key==="Enter"&&!l.shiftKey&&(l.preventDefault(),e.requestSubmit())}),a==null||a.addEventListener("input",()=>{a.style.height="auto",a.style.height=Math.min(140,a.scrollHeight)+"px"}),t.querySelectorAll(".lib-chat-sug-btn").forEach(l=>l.addEventListener("click",()=>ft(l.dataset.sug))),t.querySelectorAll(".lib-chat-apply").forEach(l=>{const i=l.querySelector(".btn");i==null||i.addEventListener("click",async()=>{try{const c=JSON.parse(l.dataset.action);await Vt(c),i.disabled=!0,i.textContent="✓ Feito"}catch(c){x("Erro ao aplicar: "+c.message,"err")}})})}async function Vt(t){const e=t.payload||{};switch(t.tipo){case"fases":{if(!Array.isArray(e))throw new Error("payload inválido");d.fases=e.filter(s=>Number.isFinite(+s.inicio)&&Number.isFinite(+s.aporte)).map(s=>({inicio:+s.inicio,aporte:+s.aporte})).sort((s,o)=>s.inicio-o.inicio);const a=d.fases[d.fases.length-1];a&&a.inicio+6>d.meses&&(d.meses=Math.min(240,a.inicio+24)),x("Fases aplicadas no simulador 🎯"),q(),setTimeout(()=>{var s;return(s=document.querySelector(".lib-sim"))==null?void 0:s.scrollIntoView({behavior:"smooth",block:"center"})},100);return}case"caixinha-create":{if(!e.nome||!(+e.valor_mensal>0))throw new Error("nome e valor_mensal obrigatórios");const a={nome:e.nome,valor_mensal:+e.valor_mensal,tipo:e.tipo==="reserva"?"reserva":"gasto",icone:e.icone||"💰",cor:e.cor||"#4A9EFF"},{error:s}=await w.from("caixinhas").insert(a);if(s)throw s;x(`Caixinha "${e.nome}" criada 📦`),await K(),D();return}case"caixinha-update":{if(!e.id)throw new Error("id obrigatório");const a={atualizado_em:new Date().toISOString()};e.nome&&(a.nome=e.nome),e.valor_mensal!=null&&(a.valor_mensal=+e.valor_mensal),e.tipo&&(a.tipo=e.tipo==="reserva"?"reserva":"gasto"),e.icone&&(a.icone=e.icone),e.cor&&(a.cor=e.cor);const{error:s}=await w.from("caixinhas").update(a).eq("id",e.id);if(s)throw s;x("Caixinha atualizada ✏️"),await K(),D();return}case"caixinha-mov":{if(!e.caixinha_id||!(+e.valor>0))throw new Error("caixinha_id e valor obrigatórios");const a={caixinha_id:e.caixinha_id,valor:+e.valor,data:e.data||new Date().toISOString().slice(0,10),descricao:e.descricao||null},{error:s}=await w.from("caixinhas_mov").insert(a);if(s)throw s;x("Gasto registrado 💸"),await K(),D();return}case"caixinha-delete":{if(!e.id)throw new Error("id obrigatório");if(!confirm(`Excluir caixinha "${e.nome||""}"? Movimentações também serão removidas.`))return;const{error:a}=await w.from("caixinhas").delete().eq("id",e.id);if(a)throw a;x("Caixinha excluída"),await K(),D();return}default:throw new Error("Tipo de ação desconhecido: "+t.tipo)}}function Yt(){const t=_.find(s=>s.principal)||_[0],e=_.filter(s=>s!==t),a=_.find(s=>s.id===d.metaId)||t;return`
  <div class="lib">
    ${t?te(t):Zt()}
  ...
  </button>`}function St(){if(!E.open)return'<div id="lib-chat" class="lib-chat"></div>';const t=E.msgs.length?E.msgs.map(e=>Wt(e)).join(""):`<div class="lib-chat-empty">
  ...
  </div>`}function Wt(t){const e=Ut(t.content),a=t.content.replace(/```apply-[a-z-]+[\s\S]*?```/g,"").trim(),s=Xt(a),o=t.role==="user"?"user":"ai",n=e.map((l,i)=>{const c=Qt(l);return`
      <div class="lib-chat-apply" data-action='${Kt(JSON.stringify(l))}'>
  ...
  </div>`}function te(t){const e=Q(t),a=Number(t.valor_alvo),s=Math.min(100,e/a*100),o=J(),n=Tt(t.id),l=Pt(t.id),i=at({pv:e,fases:[{inicio:0,aporte:o}],taxaAA:Number(k.selic_aa),meta:a});return`
  <div class="lib-hero" style="--meta-color:${t.cor||"#C5F82A"}">
    <div class="lib-hero-left">
      ...
      <div class="lib-hero-chips">
        ...
        <span class="lib-chip"><b>Aporte sug.</b> ${y(o)}/mês <small>(${Number(k.aporte_pct_faturamento)}% do fat)</small></span>
  ...
  </div>`}function ee(t){const e=Q(t),a=Number(t.valor_alvo),s=Math.min(100,e/a*100),o=t.cor||"#C5F82A",n=J(),l=at({pv:e,fases:[{inicio:0,aporte:n}],taxaAA:Number(k.selic_aa),meta:a}),i=e>=a;return`
  ...
  </div>`}function ae(t){if(!t)return"";const e=Q(t),a=Number(t.valor_alvo),s=dt({pv:e,fases:d.fases,taxaAA:d.taxa,meses:d.meses}),o=s[s.length-1];o.aportado;const n=o.juros,l=at({pv:e,fases:d.fases,taxaAA:d.taxa,meta:a});return`
  <div class="lib-stats" id="lib-stats">
    <div class="lib-stat">
      <div class="lib-stat-key">Saldo da meta</div>
      <div class="lib-stat-val">${y(e)}</div>
      <div class="lib-stat-sub">${rt(t).length} aporte${rt(t).length===1?"":"s"} registrado${rt(t).length===1?"":"s"}</div>
    </div>
    <div class="lib-stat">
      <div class="lib-stat-key">Faturamento médio 6m</div>
      <div class="lib-stat-val">${y(ut())}</div>
      <div class="lib-stat-sub">Mês atual: ${y(mt())}</div>
    </div>
    <div class="lib-stat">
    ...
    <div class="lib-stat">
      ...
      <div class="lib-stat-sub">${l!=null?`Bate em ${st(l)}`:"Aumente o aporte"}</div>
    </div>
  </div>`}function se(t){if(!t)return"";const e=Q(t),a=Number(t.valor_alvo),s=dt({pv:e,fases:d.fases,taxaAA:d.taxa,meses:d.meses}),o=J()||1e3,n=d.fases.map((l,i)=>`
  ...
  <div class="lib-ring">
    ...
    <div class="lib-ring-inner">
  ...
  </div>`}function Ft(t,e,a=null){const n={t:38,r:90,b:32,l:56},l=760-n.l-n.r,i=290-n.t-n.b,c=Math.max(e,...t.map(m=>m.saldo))*1.08,p=m=>n.l+m/(t.length-1)*l,b=m=>n.t+i-Math.max(0,m)/c*i;function h(m){const v=t.map((C,Z)=>[p(Z),b(C[m])]);if(v.length<2)return"";let F=`M ${v[0][0].toFixed(1)} ${v[0][1].toFixed(1)}`;const M=.18;for(let C=0;C<v.length-1;C++){const Z=v[C-1]||v[C],tt=v[C],H=v[C+1],pt=v[C+2]||H,qt=tt[0]+(H[0]-Z[0])*M,jt=tt[1]+(H[1]-Z[1])*M,Nt=H[0]-(pt[0]-tt[0])*M,Ct=H[1]-(pt[1]-tt[1])*M;F+=` C ${qt.toFixed(1)} ${jt.toFixed(1)}, ${Nt.toFixed(1)} ${Ct.toFixed(1)}, ${H[0].toFixed(1)} ${H[1].toFixed(1)}`}return F}const g=h("saldo"),A=h("aportado"),u=h("juros"),$=p(t.length-1),j=b(0),T=`${g} L ${$.toFixed(1)} ${j.toFixed(1)} L ${n.l} ${j.toFixed(1)} Z`,V=[0,.5,1].map(m=>{const v=c*m;return`<g>
      <line x1="${n.l}" x2="${760-n.r}" y1="${b(v)}" y2="${b(v)}" stroke="rgba(255,255,255,.04)" stroke-dasharray="${m===0?"":"2 4"}"/>
      <text x="${n.l-10}" y="${b(v)+3}" font-size="10" fill="rgba(255,255,255,.4)" text-anchor="end" font-family="JetBrains Mono" font-weight="500">${W(v)}</text>
    </g>`}).join(""),R=[0,Math.floor(t.length/2),t.length-1].map(m=>{const v=t[m];return`<text x="${p(m)}" y="280" font-size="10" fill="rgba(255,255,255,.4)" text-anchor="middle" font-family="JetBrains Mono" font-weight="500">${v.mes}m</text>`}).join("");let O="";a&&a.length>1&&(O=a.slice(1).map(m=>{if(m.inicio<=0||m.inicio>=t.length)return"";const v=p(m.inicio);return`
        ...
        <g transform="translate(${v} ${n.t-14})">
          ...
          <text x="0" y="3" font-size="9.5" font-weight="700" fill="#C7B3FB" text-anchor="middle" font-family="JetBrains Mono">→ ${W(m.aporte)}/m</text>
      ...
      <g transform="translate(${760-n.r+4} ${m})">
        ...
        <text x="39" y="3.5" font-size="10" fill="rgba(255,255,255,.7)" text-anchor="middle" font-family="JetBrains Mono" font-weight="600">META ${W(e)}</text>
        ...
        <g class="lib-chart-cross" transform="translate(${F} ${M})">
          ...
          <g transform="translate(0 -22)">
            ...
            <text x="0" y="2" font-size="10.5" font-weight="700" fill="#0A0F0A" text-anchor="middle" font-family="JetBrains Mono">🎯 ${t[v].mes}m · ${st(t[v].mes).toUpperCase()}</text>
          </g>
        </g>`}}const z=t[t.length-1],S=`
    <g transform="translate(${$+8} ${b(z.saldo)})">
      <text font-size="11" font-weight="700" fill="#C5F82A" font-family="JetBrains Mono" dy="3">${W(z.saldo)}</text>
    </g>
    <g transform="translate(${$+8} ${b(z.aportado)})">
      <text font-size="10" fill="#4A9EFF" font-family="JetBrains Mono" dy="3">${W(z.aportado)}</text>
    </g>
    <g transform="translate(${$+8} ${b(z.juros)})">
      <text font-size="10" fill="#A78BFA" font-family="JetBrains Mono" dy="3">${W(z.juros)}</text>
  ...
  </div>`}function Mt(t){const e=t.querySelector(".lib-chart"),a=t.querySelector(".lib-chart-tooltip");if(!e||!a)return;const s=e.querySelector(".lib-chart-hover"),o=e.querySelector(".lib-chart-hit"),n=e.querySelector(".lib-chart-vline"),l=e.querySelector(".lib-chart-dot-s"),i=e.querySelector(".lib-chart-dot-a"),c=e.querySelector(".lib-chart-dot-j"),p=e.dataset.xs.split(",").map(Number),b=e.dataset.saldo.split(",").map(Number),h=e.dataset.aportado.split(",").map(Number),g=e.dataset.juros.split(",").map(Number),A=e.dataset.meses.split(",").map(Number),u=+e.dataset.w,$=+e.dataset.h,j=+e.dataset.padT,T=+e.dataset.padB,V=Math.max(...b)*1.08,P=$-j-T,R=O=>j+P-Math.max(0,O)/V*P;o.addEventListener("mousemove",O=>{const I=e.getBoundingClientRect(),Y=u/I.width,z=(O.clientX-I.left)*Y;let S=0,G=1/0;for(let m=0;m<p.length;m++){const v=Math.abs(p[m]-z);v<G&&(G=v,S=m)}const N=p[S];s.style.opacity=1,n.setAttribute("x1",N),n.setAttribute("x2",N),l.setAttribute("cx",N),l.setAttribute("cy",R(b[S])),i.setAttribute("cx",N),i.setAttribute("cy",R(h[S])),c.setAttribute("cx",N),c.setAttribute("cy",R(g[S]));const r=N/u*I.width,f=r>I.width/2?"left":"right";a.style.opacity=1,a.style.left=f==="right"?r+14+"px":r-14-a.offsetWidth+"px",a.style.top=Math.max(8,R(b[S])/$*I.height-a.offsetHeight/2)+"px",a.querySelector(".lib-tt-mes").textContent=`${A[S]}m · ${st(A[S])}`,a.querySelector(".lib-tt-s").textContent=y(b[S]),a.querySelector(".lib-tt-a").textContent=y(h[S]),a.querySelector(".lib-tt-j").textContent=y(g[S])}),o.addEventListener("mouseleave",()=>{s.style.opacity=0,a.style.opacity=0})}function ie(){return`
  <div class="lib-block">
    <div class="lib-block-head">
      <h3>Histórico de aportes</h3>
      <button class="btn bp bsm" data-aporte="">+ Aporte</button>
    </div>
    ${B.length?re():'<div class="empty" style="padding:34px;text-align:center;color:var(--text-3)">Nenhum aporte ainda. Bora começar!</div>'}
  </div>`}function re(){const t={};for(const e of B){const a=(e.data||"").slice(0,7);t[a]||(t[a]=[]),t[a].push(e)}return Object.entries(t).sort((e,a)=>a[0].localeCompare(e[0])).map(([e,a])=>{const[s,o]=e.split("-"),n=a.reduce((l,i)=>l+Number(i.valor||0),0);return`
    <div class="lib-mes">
      ...
      ${a.map(l=>{const i=_.find(c=>c.id===l.meta_id);return`
        <div class="lib-ap-row" data-ap-edit="${l.id}" title="Clica pra editar">
          <div class="lib-ap-data">${Bt(l.data)}</div>
          <div class="lib-ap-meta">${i?`<span class="lib-ap-tag" style="background:${It(i.cor||"#C5F82A",.15)};color:${i.cor||"#C5F82A"}">${i.icone||"🎯"} ${L(i.nome)}</span>`:'<span class="lib-ap-tag" style="background:rgba(255,255,255,.05);color:var(--text-3)">sem meta</span>'}
          </div>
    ...
    </div>`}).join("")}function ne(t){var e,a,s,o,n,l;t.querySelectorAll("#lib-add-meta, #lib-add-meta-2").forEach(i=>i.addEventListener("click",()=>ht())),(e=t.querySelector("#sim-taxa"))==null||e.addEventListener("input",i=>X({taxa:+i.target.value})),(a=t.querySelector("#sim-meses"))==null||a.addEventListener("input",i=>X({meses:+i.target.value})),(s=t.querySelector("#sim-meta"))==null||s.addEventListener("change",i=>{d.metaId=i.target.value,q()}),(o=t.querySelector("#lib-fases-list"))==null||o.addEventListener("input",i=>{const c=i.target.closest(".lib-fase");if(!c)return;const p=+c.dataset.faseIdx;if(d.fases[p]){if(i.target.classList.contains("lib-fase-aporte"))d.fases[p].aporte=Math.max(0,+i.target.value||0),X({});else if(i.target.classList.contains("lib-fase-inicio")){const b=Math.max(1,+i.target.value||1);if(d.fases[p].inicio=b,b+6>d.meses){d.meses=Math.min(240,b+24),q();return}X({})}}}),(n=t.querySelector("#lib-fase-add"))==null||n.addEventListener("click",()=>{const i=d.fases[d.fases.length-1],c=((i==null?void 0:i.inicio)||0)+12,p=Math.round(((i==null?void 0:i.aporte)||2e3)*1.5);d.fases.push({inicio:c,aporte:p}),c+6>d.meses&&(d.meses=Math.min(240,c+24)),q(),setTimeout(()=>{var b;return(b=document.querySelector(".lib-fase:last-child .lib-fase-aporte"))==null?void 0:b.focus()},50)}),(l=t.querySelector("#lib-fases-list"))==null||l.addEventListener("click",i=>{const c=i.target.closest(".lib-fase-del");if(!c)return;const b=+c.closest(".lib-fase").dataset.faseIdx;b!==0&&(d.fases.splice(b,1),q())}),t.querySelectorAll(".lib-qk").forEach(i=>i.addEventListener("click",()=>{d.fases[0].aporte=+i.dataset.pmt,q()})),t.addEventListener("click",async i=>{const c=i.target.closest("[data-aporte]"),p=i.target.closest("[data-sim]"),b=i.target.closest("[data-meta-edit]"),h=i.target.closest(".lib-ap-del"),g=i.target.closest("#lib-selic-refresh");if(g){i.stopPropagation(),g.classList.add("spinning");const u=await ct({manual:!0});if(g.classList.remove("spinning"),u){d.taxa=Number(k.selic_aa);const $=document.getElementById("sim-taxa");$&&document.activeElement!==$&&($.value=d.taxa,X({taxa:d.taxa}))}return}if(c){vt({metaId:c.dataset.aporte||null});return}if(p){d.metaId=p.dataset.sim,q(),setTimeout(()=>{var u;return(u=document.querySelector(".lib-sim"))==null?void 0:u.scrollIntoView({behavior:"smooth",block:"center"})},50);return}if(b){const u=_.find($=>$.id===b.dataset.metaEdit);u&&ht(u);return}if(h){if(i.stopPropagation(),!confirm("Excluir esse aporte?"))return;const{error:u}=await w.from("aportes_fin").delete().eq("id",h.dataset.aid);if(u)return x("Erro: "+u.message,"err");x("Aporte removido"),q();return}const A=i.target.closest("[data-ap-edit]");if(A){const u=B.find($=>$.id===A.dataset.apEdit);u&&vt({aporte:u})}})}function X(t){Object.assign(d,t);const e=document.getElementById("content"),a=_.find(A=>A.id===d.metaId)||_.find(A=>A.principal)||_[0];if(!a)return;const s=e.querySelectorAll(".lib-slider-v");s[0]&&(s[0].textContent=d.taxa.toFixed(2)+"% a.a."),s[1]&&(s[1].textContent=`${d.meses} meses (${(d.meses/12).toFixed(1)} anos)`);const o=Q(a),n=Number(a.valor_alvo),l=dt({pv:o,fases:d.fases,taxaAA:d.taxa,meses:d.meses}),i=l[l.length-1],c=i.aportado,p=e.querySelector(".lib-sim-chart");if(p){const A=p.querySelector(".lib-chart-legend");p.innerHTML=Ft(l,n,d.fases)+(A?A.outerHTML:""),Mt(p)}const b=e.querySelectorAll("#lib-stats .lib-stat-val"),h=e.querySelectorAll("#lib-stats .lib-stat-sub");b[2]&&(b[2].textContent=y(i.saldo),h[2].textContent="Juros: "+y(i.saldo-c));const g=at({pv:o,fases:d.fases,taxaAA:d.taxa,meta:n});b[3]&&(b[3].textContent=y(Math.max(0,n-o)),h[3].textContent=g!=null?`Bate em ${st(g)}`:"Aumente o aporte")}function vt({metaId:t=null,aporte:e=null}={}){var h,g,A;const a=new Date().toISOString().slice(0,10),s=J(),o=!!e,n=o?e.meta_id||"":t||((h=_.find(u=>u.principal))==null?void 0:h.id)||((g=_[0])==null?void 0:g.id)||"",l=o?e.valor:s||"",i=o?e.data:a,c=o&&e.fonte||"manual",p=o&&e.observacao||"",b=["manual","faturamento","bonus","outro"].map(u=>`<option value="${u}"${u===c?" selected":""}>${u==="manual"?"Manual":u==="faturamento"?"Faturamento":u==="bonus"?"Bônus / Extra":"Outro"}</option>`).join("");gt(o?"Editar aporte":"Registrar aporte",`
    ...
    <div class="fg"><label class="fl">Valor</label>
      ...
      ${!o&&s?`<div class="lib-hint">Sugestão: ${y(s)} (${k.aporte_pct_faturamento}% do faturamento médio)</div>`:""}
    </div>
  ...
  `),document.getElementById("ap-cancel").addEventListener("click",U),(A=document.getElementById("ap-del"))==null||A.addEventListener("click",async()=>{if(!confirm("Excluir esse aporte?"))return;const{error:u}=await w.from("aportes_fin").delete().eq("id",e.id);if(u)return x("Erro: "+u.message,"err");U(),x("Aporte removido"),q()}),document.getElementById("ap-save").addEventListener("click",async()=>{const u=parseFloat(document.getElementById("ap-val").value)||0;if(u<=0)return x("Valor inválido","err");const $={valor:u,data:document.getElementById("ap-data").value||a,fonte:document.getElementById("ap-fonte").value,observacao:document.getElementById("ap-obs").value.trim()||null,meta_id:document.getElementById("ap-meta").value||null},{error:j}=o?await w.from("aportes_fin").update($).eq("id",e.id):await w.from("aportes_fin").insert($);if(j)return x("Erro: "+j.message,"err");U(),x(o?"Aporte atualizado":"Aporte registrado 🚀"),q()})}function ht(t={}){var l;const e=!t.id,a=t.icone||bt[0],s=t.cor||it[_.length%it.length],o=bt.map(i=>`<button type="button" class="lib-ico-pick${i===a?" on":""}" data-ico="${i}">${i}</button>`).join(""),n=it.map(i=>`<button type="button" class="lib-cor-pick${i===s?" on":""}" style="background:${i}" data-cor="${i}"></button>`).join("");gt(e?"Nova meta":"Editar meta",`
  ...
  `),document.getElementById("m-icones").addEventListener("click",i=>{const c=i.target.closest(".lib-ico-pick");c&&(document.querySelectorAll(".lib-ico-pick").forEach(p=>p.classList.remove("on")),c.classList.add("on"),document.getElementById("m-ico").value=c.dataset.ico)}),document.getElementById("m-cores").addEventListener("click",i=>{const c=i.target.closest(".lib-cor-pick");c&&(document.querySelectorAll(".lib-cor-pick").forEach(p=>p.classList.remove("on")),c.classList.add("on"),document.getElementById("m-cor").value=c.dataset.cor)}),document.getElementById("m-cancel").addEventListener("click",U),(l=document.getElementById("m-del"))==null||l.addEventListener("click",async()=>{if(!confirm(`Excluir meta "${t.nome}"? Aportes vinculados ficam sem meta.`))return;const{error:i}=await w.from("metas_fin").delete().eq("id",t.id);if(i)return x("Erro: "+i.message,"err");U(),x("Meta excluída"),q()}),document.getElementById("m-save").addEventListener("click",async()=>{const i=document.getElementById("m-nome").value.trim(),c=parseFloat(document.getElementById("m-alvo").value)||0;if(!i)return x("Nome obrigatório","err");if(c<=0)return x("Valor alvo inválido","err");const p=document.getElementById("m-principal").checked,b={nome:i,valor_alvo:c,valor_inicial:parseFloat(document.getElementById("m-ini").value)||0,prazo_meses:parseInt(document.getElementById("m-prazo").value)||null,cor:document.getElementById("m-cor").value,icone:document.getElementById("m-ico").value,principal:p,atualizado_em:new Date().toISOString()};p&&await w.from("metas_fin").update({principal:!1}).neq("id",t.id||"00000000-0000-0000-0000-000000000000");const{error:h}=t.id?await w.from("metas_fin").update(b).eq("id",t.id):await w.from("metas_fin").insert(b);if(h)return x("Erro: "+h.message,"err");U(),x(t.id?"Meta atualizada":"Meta criada 🎯"),q()})}function le(t){return Math.round(t).toLocaleString("pt-BR")}function W(t){return t>=1e6?(t/1e6).toFixed(1).replace(/\.0$/,"")+"M":t>=1e3?(t/1e3).toFixed(1).replace(/\.0$/,"")+"k":Math.round(t).toString()}function st(t){const e=new Date;return e.setMonth(e.getMonth()+t),$t[e.getMonth()]+"/"+e.getFullYear()}function L(t){return String(t||"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function It(t,e){const a=t.replace("#",""),s=parseInt(a.slice(0,2),16),o=parseInt(a.slice(2,4),16),n=parseInt(a.slice(4,6),16);return`rgba(${s},${o},${n},${e})`}function ce(t,e){const a=t.replace("#",""),s=Math.min(255,parseInt(a.slice(0,2),16)+e),o=Math.min(255,parseInt(a.slice(2,4),16)+e),n=Math.min(255,parseInt(a.slice(4,6),16)+e);return`rgb(${s},${o},${n})`}function de(t,e){const a=t.replace("#",""),s=Math.max(0,parseInt(a.slice(0,2),16)-e),o=Math.max(0,parseInt(a.slice(2,4),16)-e),n=Math.max(0,parseInt(a.slice(4,6),16)-e);return`rgb(${s},${o},${n})`}export{q as render};

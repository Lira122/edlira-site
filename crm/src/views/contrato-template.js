// HTML completo do contrato de prestação de serviços da Eleva Digital.
// Recebe os dados preenchidos e devolve uma string HTML pronta pra abrir em nova
// aba e imprimir/salvar como PDF.

export function buildContratoHTML(d) {
  const esc = s => String(s ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')

  // Defaults pros campos nunca virem como undefined no documento
  const razao         = esc(d.razaoSocial || '____________________')
  const cnpj          = esc(d.cnpj         || '____________________')
  const endereco      = esc(d.endereco     || '____________________')
  const representante = esc(d.representante || '____________________')
  const email         = esc(d.email        || '____________________')
  const valor         = esc(d.valor || '____')
  const valorExt      = esc(d.valorExtenso || '____________________')
  const diaVenc       = esc(d.diaVencimento || '__')
  const valorMidia    = esc(d.valorMidia || '____')
  const cidade        = esc(d.cidade || 'Taubaté')
  const dataDia       = esc(d.dataDia || '__')
  const dataMes       = esc(d.dataMes || '____________')
  const dataAno       = esc(d.dataAno || new Date().getFullYear())
  const docNum        = esc(d.docNum || '001')
  const autorizado    = d.autorizaPortfolio === true

  return `<!doctype html>
<html lang="pt-BR">
<head>
<meta charset="utf-8">
<title>Contrato · ${razao}</title>
...
<div class="page cover">
  <div class="cover-top">
    ...
    <div class="cover-doc">Documento<b>Nº ${docNum} · ${dataAno}</b></div>
  </div>
  ...
  <div class="cover-mid">
    ...
    <div class="cover-info">
      ...
      <div>
        ...
        <div class="v"><span class="marker">${razao}</span><small>CNPJ <span class="marker">${cnpj}</span></small></div>
      </div>
      <div>
...
<div class="page">
  ...
  <div class="preamble">
    Pelo presente instrumento particular, de um lado, <span class="marker">${razao}</span>, pessoa jurídica de direito privado, inscrita no CNPJ sob nº <span class="marker">${cnpj}</span>, com sede em <span class="marker">${endereco}</span>, neste ato representada por <span class="marker">${representante}</span> (<span class="marker">${email}</span>), doravante denominada simplesmente CONTRATANTE, e, de outro lado, <b>EDMILSON ROSA LIRA JUNIOR</b>, brasileiro, solteiro, portador do RG nº 29.389.286-5, inscrito no CPF nº 181.565.527-50, residente e domiciliado na Avenida Monte Castelo, nº 156, Vila Jaboticabeira, Taubaté – SP, CEP: 12030-660, atuando sob a marca <b>ELEVA DIGITAL</b>, doravante denominado CONTRATADO, têm entre si justo e contratado o que segue.
  </div>
...
<div class="page">
  ...
  <div class="clause">
    ...
    <div class="clause-side">
      <p>A CONTRATANTE <span class="check ${autorizado ? 'on' : ''}"><span class="box"></span>AUTORIZA</span> <span class="check ${autorizado ? '' : 'on'}"><span class="box"></span>NÃO AUTORIZA</span> o CONTRATADO a mencionar a prestação dos serviços e a utilizar resultados, métricas, peças e demonstrações desenvolvidas como referência de <b>portfólio</b>, em materiais comerciais, redes sociais e conteúdos institucionais da marca ELEVA DIGITAL.</p>
...
<div class="page">
  ...
  <div class="sign-date">
    <span>${cidade}</span>, <span>${dataDia}</span> de <span>${dataMes}</span> de 20<span>${String(dataAno).slice(-2)}</span>.
  </div>

  <div class="sign-pair">
    <div class="sign-col">
      <div class="line"></div>
      <div class="name"><span class="marker">${representante}</span></div>
